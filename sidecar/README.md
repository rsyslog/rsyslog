...
A lightweight Python sidecar process that reads rsyslog `impstats` files and exposes metrics in Prometheus format via an HTTP endpoint.
...
> **⚠️ Production Deployments**: For production environments, **always use the gunicorn WSGI server** instead of the development server. See [PRODUCTION.md](PRODUCTION.md) for complete production deployment guide.
...
1. **Install the script:**
   ```bash
   sudo cp rsyslog_exporter.py /usr/local/bin/
   sudo chmod +x /usr/local/bin/rsyslog_exporter.py
   ```
...
3. **Install and configure systemd service:**
   ```bash
   sudo cp rsyslog-exporter.service /etc/systemd/system/
...
|----------|---------|-------------|
| `IMPSTATS_MODE` | `file` | Input mode: `file` or `udp` |
...
| `REFRESH_INTERVAL` | `5` | Background refresh interval (file mode only, seconds) |
| `LOG_LEVEL` | `INFO` | Logging level: `DEBUG`, `INFO`, `WARNING`, `ERROR` |
| `ALLOWED_UDP_SOURCES` | (unset) | Optional comma-separated IP allowlist for UDP packets (UDP mode). If set, packets from other sources are dropped and counted in `/health` as `dropped_messages`. |
...
- **Exporter refresh**: 5 seconds (checks file mtime, reloads only if changed)
...
- Increase refresh interval to reduce parsing frequency

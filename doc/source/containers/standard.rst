...
The inherited regex lookup-table noise filter from the minimal image is
available at ``/etc/rsyslog/noise-drop.lkp_tbl``. It matches ``$rawmsg``
by default and drops events when the lookup result is non-empty.

...
Limits the decompressed-to-compressed byte ratio accepted from one TCP receive.
...
accepts relative to compressed bytes received in one TCP receive operation when
``compression.mode="stream:always"`` is enabled. The default allows up to
1024 decompressed bytes per compressed byte.

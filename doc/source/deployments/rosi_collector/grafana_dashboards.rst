...
.. figure:: /_static/dashboard-health.png
   :alt: Client Health Dashboard
   :align: center
   
   Client Health monitoring rsyslog statistics

Monitor rsyslog client status and queue health using impstats metrics:

**Overview Panels**:

- **Syslog Clients Reporting** - Count of clients sending impstats data
- **Action Failures** - Total failed message processing attempts
- **Queue Full Events** - Times a queue reached capacity
- **Stats Messages** - Total impstats messages received

**Time Series Panels**:

- **Queue Depth** - Current queue size per client (thresholds: warning at 1000, critical at 5000)
- **Messages Processed** - Rate of log forwarding per client
- **Memory Usage (RSS)** - rsyslog process memory consumption

**Metrics collected from impstats**:

+------------------+----------------+----------------------------------+
| Metric           | Source         | Description                      |
+==================+================+==================================+
| ``size``         | core.queue     | Current queue depth              |
+------------------+----------------+----------------------------------+
| ``full``         | core.queue     | Queue full events                |
+------------------+----------------+----------------------------------+
| ``processed``    | core.action    | Messages forwarded               |
+------------------+----------------+----------------------------------+
| ``failed``       | core.action    | Action failures                  |
+------------------+----------------+----------------------------------+
| ``maxrss``       | resource-usage | Memory usage (KB)                |
+------------------+----------------+----------------------------------+
| ``openfiles``    | resource-usage | Open file descriptors            |
+------------------+----------------+----------------------------------+

A growing queue indicates connectivity problems. Check the
:doc:`troubleshooting` guide if queues are backing up.

**Enabling impstats on clients**:

The automated client installation script offers to enable impstats. See
:doc:`client_setup` for manual configuration.

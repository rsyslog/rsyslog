...
Optional: rsyslog Statistics
----------------------------

For detailed rsyslog monitoring, enable impstats. The automated
``install-rsyslog-client.sh`` script prompts to enable this automatically.

To enable manually:

.. code-block:: bash

   sudo nano /etc/rsyslog.d/10-impstats.conf
...
- Messages processed and bytes sent
- Action failures
- Memory usage (RSS) and open file descriptors

Statistics appear in logs and are visible in the Client Health dashboard.

...
- Keep the `development/coding_practices` page discoverable; it is the canonical
  source for patterns and antipatterns that AI agents must ingest.
...
- Prime code-generation agents with the `development/coding_practices` page so
  RAG pipelines can inject the right patterns and antipatterns into their
  context.

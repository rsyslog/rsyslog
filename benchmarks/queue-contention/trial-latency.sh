...
python3 /campaign/latency-observer.py --host 127.0.0.1 --port "$INPUT_PORT" --output "$RSYSLOG_OUT_LOG" \
    --result "$BENCH_METRIC_FILE" --messages "$NUMMESSAGES" --connections "$BENCH_CONNECTIONS" --rate "$BENCH_OFFERED_RATE" --poll-us "$BENCH_POLL_US"
shutdown_when_empty
wait_shutdown
exit_test

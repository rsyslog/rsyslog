...
-  |FmtAdvancedName| - previously known as "``RainerScript``",
...
-  |FmtObsoleteName| - previously known simply as the ``legacy``
   ...
   are encouraged to migrate to the |FmtBasicName| or
   |FmtAdvancedName| formats.

Which Format should I Use?
~~~~~~~~~~~~~~~~~~~~~~~~~~

Rsyslog supports all three formats concurrently, so you can pick either
|FmtBasicName| or |FmtAdvancedName| format.

**It is recommended to use** |FmtAdvancedName| **format.** Advantages are:
...
For *very simple things* |FmtBasicName| statement types are still
...
miserable.**
The |FmtObsoleteName| format is primarily supported in order

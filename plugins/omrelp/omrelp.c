...
PRAGMA_IGNORE_Wformat_nonliteral static void __attribute__((format(printf, 1, 2))) omrelp_dbgprintf(const char *fmt,
                                                                                                    ...) {
    va_list ap;
...
static rsRetVal doCreateRelpClient(instanceData *pData, relpClt_t **pRelpClt) {
    ...
    if (relpEngineCltConstruct(pRelpEngine, pRelpClt) != RELP_RET_OK) ABORT_FINALIZE(RS_RET_RELP_ERR);
    if (relpCltSetTimeout(*pRelpClt, pData->timeout) != RELP_RET_OK) ABORT_FINALIZE(RS_RET_RELP_ERR);
    ...
    if (pData->bEnableTLS) {
        ...
        if (pData->bEnableTLSZip) {
            if (relpCltEnableTLSZip(*pRelpClt) != RELP_RET_OK) ABORT_FINALIZE(RS_RET_RELP_ERR);
        }
        ...
        if (relpCltSetAuthMode(*pRelpClt, (char *)pData->authmode) != RELP_RET_OK) {
            LogError(0, RS_RET_RELP_ERR, "omrelp: invalid auth mode '%s'\n", pData->authmode);
            ABORT_FINALIZE(RS_RET_RELP_ERR);
        }

        if (relpCltSetCACert(*pRelpClt, (char *)pData->caCertFile) != RELP_RET_OK) ABORT_FINALIZE(RS_RET_RELP_ERR);
        if (relpCltSetOwnCert(*pRelpClt, (char *)pData->myCertFile) != RELP_RET_OK) ABORT_FINALIZE(RS_RET_RELP_ERR);
        if (relpCltSetPrivKey(*pRelpClt, (char *)pData->myPrivKeyFile) != RELP_RET_OK) ABORT_FINALIZE(RS_RET_RELP_ERR);
#if defined(HAVE_RELPENGINESETTLSCFGCMD)
    ...
    if (pData->localClientIP != NULL) {
        if (relpCltSetClientIP(*pRelpClt, pData->localClientIP) != RELP_RET_OK) ABORT_FINALIZE(RS_RET_RELP_ERR);
    }
finalize_it:
...
BEGINfreeWrkrInstance
    ...
    if (pWrkrData->pRelpClt != NULL) relpEngineCltDestruct(pRelpEngine, &pWrkrData->pRelpClt);
ENDfreeWrkrInstance
...
BEGINsetModCnf
    ...
    for (i = 0; i < modpblk.nParams; ++i) {
        ...
        if (!strcmp(modpblk.descr[i].name, "tls.tlslib")) {
            ...
            dbgprintf(
                "imfile: program error, non-handled "
...
BEGINnewActInst
    ...
    for (i = 0; i < actpblk.nParams; ++i) {
        ...
        if (!strcmp(actpblk.descr[i].name, "target")) {
            ...
            dbgprintf(
                "omrelp: program error, non-handled "
                "param '%s'\n",
 ...
 */
static rsRetVal ATTR_NONNULL() doConnect(wrkrInstanceData_t *const pWrkrData) {
    DEFiRet;

    if (pWrkrData->bInitialConnect) {
        iRet = relpCltConnect(pWrkrData->pRelpClt, glbl.GetDefPFFamily(runModConf->pConf), getRelpPt(pWrkrData->pData),
                              pWrkrData->pData->target);
        if (iRet == RELP_RET_OK) pWrkrData->bInitialConnect = 0;
    } else {
...
static rsRetVal doRebind(wrkrInstanceData_t *pWrkrData) {
    DEFiRet;
...
BEGINdoAction
    ...
    if (ret != RELP_RET_OK) {
        LogError(0, RS_RET_RELP_ERR,
                 "librelp error %d%s forwarding "
                 "to server %s:%s - suspending\n",
                 ret, (ret == RELP_RET_SESSION_BROKEN) ? "[connection broken]" : "", pData->target, getRelpPt(pData));
        ABORT_FINALIZE(RS_RET_SUSPENDED);
    }

    if (pData->rebindInterval != 0 && (++pWrkrData->nSent >= pData->rebindInterval)) {
        doRebind(pWrkrData);
    }
finalize_it:
        ...
        *p = '\0'; /* trick to obtain hostname (later)! */
        ...
        if (pData->port == NULL) {
            LogError(0, NO_ERRCODE,
                     "Could not get memory to store relp port, "
    ...
    while (*p && *p != ';') {
        if (*p && *p != ';' && !isspace((int)*p)) {
            if (bErr == 0) { /* only 1 error msg! */
                ...
                errno = 0;
                LogError(0, NO_ERRCODE,
                         "invalid selector line (port), probably not doing "
                         "what was intended");
            }
        }
        ++p;
...
BEGINqueryEtryPt
    ...
    CODEqueryEtryPt_TXIF_OMOD_QUERIES CODEqueryEtryPt_SetShutdownImmdtPtr
ENDqueryEtryPt

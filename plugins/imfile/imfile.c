...
static uchar *ATTR_NONNULL(1, 2)
    OLD_getStateFileName(const instanceConf_t *const inst, uchar *const __restrict__ buf, const size_t lenbuf) {
...
static const uchar *getStateFileDir(void) {
    const uchar *wrkdir;
 ...
 */
static rsRetVal ATTR_NONNULL(1) OLD_openFileWithStateFile(act_obj_t *const act) {
    DEFiRet;
    strm_t *psSF = NULL;
    ...
    if (stat((char *)pszSFNam, &stat_buf) == -1) {
        if (errno == ENOENT) {
            ...
            DBGPRINTF(
                "OLD_openFileWithStateFile: error trying to access state "
                "file for '%s':%s\n",
                act->name, errStr);
            ABORT_FINALIZE(RS_RET_IO_ERROR);
        }
...
finalize_it:
    if (psSF != NULL) strm.Destruct(&psSF);
    RETiRet;
}
...
#ifdef HAVE_INOTIFY_INIT
    #if ULTRA_DEBUG == 1
static void dbg_wdmapPrint(const char *msg) {
    int i;
    DBGPRINTF("%s\n", msg);
    for (i = 0; i < nWdmap; ++i)
...
static rsRetVal wdmapInit(void) {
    DEFiRet;
    free(wdmap);
...
static rsRetVal wdmapAdd(int wd, act_obj_t *const act) {
    wd_map_t *newmap;
    int newmapsize;
    int i;
    DEFiRet;

    for (i = nWdmap - 1; i >= 0 && wdmap[i].wd > wd; --i); /* just scan */
...
static int in_setupWatch(act_obj_t *const act, const int is_file) {
    int wd = -1;
...
static int wdmap_cmp(const void *k, const void *a) {
    int key = *((int *)k);
 ...
 * or -1 if not found.
 */
static wd_map_t *wdmapLookup(int wd) {
...
static rsRetVal wdmapDel(const int wd) {
    ...
    for (i = 0; i < nWdmap && wdmap[i].wd < wd; ++i); /* just scan */
...
static void ATTR_NONNULL() fen_setupWatch(act_obj_t *const act) {
    ...
    if (runModConf->opMode != OPMODE_FEN) goto done;
    ...
    if (act->pfinf == NULL) {
        ...
        if (act->pfinf == NULL) {
            LogError(errno, RS_RET_OUT_OF_MEMORY,
                     "imfile: fen_setupWatch alloc memory "
                     "for fileinfo failed ");
            goto done;
        }
...
#else
static void ATTR_NONNULL() fen_setupWatch(act_obj_t *const act __attribute__((unused))) {
    DBGPRINTF("fen_setupWatch: DUMMY CALLED - not on Solaris?\n");
}
#endif /* FEN */

static void fs_node_print(const fs_node_t *const node, const int level) {
    fs_edge_t *chld;
    act_obj_t *act;
...
static sbool isIgnoreOlderFile(const instanceConf_t *const inst, const char *const name) {
 ...
 * if it already does.
 */
static rsRetVal ATTR_NONNULL(1, 2) act_obj_add(fs_edge_t *const edge,
                                               const char *const name,
                                               const int is_file,
                                               const ino_t ino,
                                               const int is_symlink,
    ...
    if (is_file && !is_symlink) {
        ...
        CHKiRet(ratelimitNewWithThreadMode(&act->ratelimiter, "imfile", name, 1));
...
static void detect_updates(fs_edge_t *const edge) {
    ...
    do {
        restart = 0;
        for (act = edge->active; act != NULL; act = act->next) {
            /* Snapshot act->name before list modifications. act->name remains valid
             * until act_obj_destroy() frees it (after the do-while loop), but a
             * local alias keeps the log calls below simple and avoids relying on
            ...
            if (r == -1) { /* object gone away? */
                ...
                if (!is_file || act->time_to_delete + FILE_DELETE_DELAY < ttNow) {
                     ...
                     */
                    act->deferred_parent_update = act->is_symlink ? find_symlink_parent_update(act) : NULL;
...
static rsRetVal ATTR_NONNULL() process_symlink(fs_edge_t *const chld, const char *symlink) {
    DEFiRet;
    char *target;
    ...
    if (ret == 0) {
        ...
        for (unsigned i = 0; i < files.gl_pathc; i++) {
            ...
            if (lstat(file, &fileInfo) != 0) {
                LogError(errno, RS_RET_ERR, "imfile: poll_tree cannot stat file '%s' - ignored", file);
                continue;
            }
            ...
            if (!issymlink && (chld->is_file != is_file)) {
                LogMsg(0, RS_RET_ERR, LOG_WARNING, "imfile: '%s' is %s but %s expected - ignored", file,
                       (is_file) ? "FILE" : "DIRECTORY", (chld->is_file) ? "FILE" : "DIRECTORY");
                continue;
            }
...
static void ATTR_NONNULL() poll_timeouts(fs_edge_t *const edge) {
    if (edge->is_file) {
        act_obj_t *act;
        for (act = edge->active; act != NULL; act = act->next) {
...
/* Return the parent edge that must be rescanned when symlink act is destroyed,
 * or NULL if no rescan is needed (act is not a symlink, or has no tracked target).
 *
 * IMPORTANT: must be called BEFORE act_obj_unlink_nodestroy(act).
 * The search scans act->edge->active for the symlink's target (identified by
 * source_name == act->name). This is safe because act_obj_add() always prepends,
 * so poll_tree() inserts the target first and the symlink second; detect_updates()
 * iterates from the list head and therefore reaches the symlink before the target.
 * If that insertion order ever changes, this function must be revisited.
 *
 * Callers already guard with act->is_symlink; the internal check is kept for
 * defensive safety in case the function is called from other contexts.
 */
static fs_edge_t *find_symlink_parent_update(act_obj_t *const act) {
    if (!act->is_symlink || act->edge == NULL) {
        return NULL;
    }
...
static void act_obj_destroy_all(act_obj_t *act) {
    if (act == NULL) return;
...
static rsRetVal fs_node_add(fs_node_t *const node,
    ...
    assert(toFind[0] != '\0');
    for (i = pathIdx; (toFind[i] != '\0') && (toFind[i] != '/'); ++i) /*JUST SKIP*/
        ;
...
static int ATTR_NONNULL() getFullStateFileName(const uchar *const pszstatefile,
    ...
    /* Construct file name */
    lenout = snprintf((char *)pszout, ilenout, "%s/%s%s%s", (char *)(pszstatedir == NULL ? "." : (char *)pszstatedir),
                      (char *)pszstatefile, (*file_id == '\0') ? "" : ":", file_id);
...
#endif
get_file_id_hash(const char *data, size_t lendata, char *const hash_str, const size_t len_hash_str) {
 ...
 */
static uchar *ATTR_NONNULL(1, 2)
    getStateFileName(const act_obj_t *const act, uchar *const __restrict__ buf, const size_t lenbuf) {
...
static rsRetVal checkPerMinuteRateLimits(per_minute_rate_limit_t *per_minute_rate_limits, const size_t msgLen) {
    DEFiRet;
 ...
 * exist or cannot be read, an error is returned.
 */
static rsRetVal ATTR_NONNULL(1) openFileWithStateFile(act_obj_t *const act) {
    DEFiRet;
    uchar pszSFNam[MAXFNAME];
    uchar statefile[MAXFNAME];
    int fd = -1;
    ...
    if (fd < 0) {
        if (errno == ENOENT) {
            ...
            LogError(errno, RS_RET_IO_ERROR, "imfile error trying to access state file for '%s'", act->name);
            ABORT_FINALIZE(RS_RET_IO_ERROR);
        }
    }
 ...
 */
static rsRetVal openFileWithoutStateFile(act_obj_t *const act) {
    DEFiRet;
    const instanceConf_t *const inst = act->edge->instarr[0];  // TODO: same file, multiple instances?

    DBGPRINTF("clean startup withOUT state file for '%s'\n", act->name);
    if (act->pStrm != NULL) strm.Destruct(&act->pStrm);
    CHKiRet(strm.Construct(&act->pStrm));
    CHKiRet(strm.SettOperationsMode(act->pStrm, STREAMMODE_READ));
    CHKiRet(strm.SetsType(act->pStrm, STREAMTYPE_FILE_MONITOR));
    CHKiRet(strm.SetFName(act->pStrm, (uchar *)act->name, strlen(act->name)));
        ...
        if (fd >= 0) {
            ...
            if (act->pStrm->iCurrOffs < 0) {
                ...
                LogError(errno, RS_RET_ERR,
                         "imfile: could not query current "
                         "file size for %s - 'freshStartTail' option will "
                         "be ignored, starting at begin of file",
                         inst->pszFileName);
            }
            CHKiRet(strm.SeekCurrOffs(act->pStrm));
        }
 ...
 */
static rsRetVal openFile(act_obj_t *const act) {
    DEFiRet;
    const instanceConf_t *const inst = act->edge->instarr[0];  // TODO: same file, multiple instances?
...
static rsRetVal ATTR_NONNULL() pollFileReal(act_obj_t *act, cstr_t **pCStr) {
    ...
    while (glbl.GetGlobalInputTermState() == 0) {
        if (inst->maxLinesAtOnce != 0 && nProcessed >= inst->maxLinesAtOnce) break;
        if ((start_preg == NULL) && (end_preg == NULL)) {
            ...
            CHKiRet(strmReadMultiLine(act->pStrm, pCStr, start_preg, end_preg, inst->escapeLF, inst->escapeLFString,
                                      inst->discardTruncatedMsg, inst->msgDiscardingError, &strtOffs));
        }
        ++nProcessed;
...
static rsRetVal ATTR_NONNULL(1) pollFile(act_obj_t *const act) {
    cstr_t *pCStr = NULL;
    DEFiRet;
    if (act->is_symlink) {
...
static rsRetVal ATTR_NONNULL() checkInstance(instanceConf_t *const inst) {
     ...
     */
    if (inst->pszFileName == NULL) ABORT_FINALIZE(RS_RET_INTERNAL_ERROR);
    ...
    if (loadModConf->normalizePath) {
        ...
        if (inst->pszFileName[0] != '/') {
            if (getcwd((char *)curr_wd, MAXFNAME) == NULL || curr_wd[0] != '/') {
                LogError(errno, RS_RET_ERR,
                         "imfile: error querying current working "
                         "directory - can not continue with %s",
                         inst->pszFileName);
                ABORT_FINALIZE(RS_RET_ERR);
            }
            ...
            if (len_curr_wd + ustrlen(inst->pszFileName) + 1 >= MAXFNAME) {
                LogError(0, RS_RET_ERR,
                         "imfile: length of configured file and current "
...
static rsRetVal addInstance(void __attribute__((unused)) * pVal, uchar *pNewVal) {
    ...
    if (cs.pszFileName == NULL) {
        LogError(0, RS_RET_CONFIG_ERROR,
                 "imfile error: no file name given, file monitor can "
                 "not be created");
        ABORT_FINALIZE(RS_RET_CONFIG_ERROR);
    }
    if (cs.pszFileTag == NULL) {
        LogError(0, RS_RET_CONFIG_ERROR,
                 "imfile error: no tag value given, file monitor can "
                 "not be created");
        ABORT_FINALIZE(RS_RET_CONFIG_ERROR);
    }
...
BEGINnewInpInst
    ...
    if (inst->readTimeout != 0) loadModConf->haveReadTimeouts = 1;
...
BEGINsetModCnf
    ...
    if (pvals == NULL) {
        LogError(0, RS_RET_MISSING_CNFPARAMS,
                 "imfile: error processing module "
    ...
    for (i = 0; i < modpblk.nParams; ++i) {
        ...
        if (!strcmp(modpblk.descr[i].name, "pollinginterval")) {
            ...
            if (!es_strconstcmp(pvals[i].val.d.estr, "polling"))
                ...
                loadModConf->opMode = OPMODE_FEN;
                DBGPRINTF(
                    "inotify mode configured, but only FEN "
...
BEGINendCnfLoad
    ...
    DBGPRINTF("opmode is %d, polling interval is %d\n", loadModConf->opMode, loadModConf->iPollInterval);
...
BEGINactivateCnf
    ...
    if (runModConf->root == NULL) {
        LogError(0, NO_ERRCODE,
                 "imfile: no file monitors configured, "
                 "input not activated.\n");
 ...
 */
static void do_initial_poll_run(void) {
    fs_node_walk(runModConf->conf_tree, poll_tree);
...
static rsRetVal doPolling(void) {
    ...
    while (glbl.GetGlobalInputTermState() == 0) {
        ...
        if (glbl.GetGlobalInputTermState() == 0) srSleep(runModConf->iPollInterval, 10);
    }
...
static void ATTR_NONNULL(1) in_dbg_showEv(const struct inotify_event *ev) {
    if (!Debug) return;
 ...
 */
static void flag_in_move(fs_edge_t *const edge, const char *name_moved) {
    act_obj_t *act;
...
static rsRetVal do_inotify(void) {
    char iobuf[8192];
    int rd;
    int currev;
    ...
    while (glbl.GetGlobalInputTermState() == 0) {
        ...
        if (r == 0) {
            ...
            LogError(errno, RS_RET_INTERNAL_ERROR, "%s:%d: ERROR: poll returned more fds (%d) than given to it (1)",
                     __FILE__, __LINE__, r);
            /* we do not abort, as this would render the whole input defunct */
            continue;
        } else {
...
static rsRetVal do_inotify(void) {
    LogError(0, RS_RET_NOT_IMPLEMENTED,
             "imfile: mode set to inotify, but the "
...
#if defined(OS_SOLARIS) && defined(HAVE_PORT_SOURCE_FILE) /* use FEN on Solaris! */
static void fen_printevent(int event) {
    ...
    while (glbl.GetGlobalInputTermState() == 0) {
        ...
        while (!port_get(glport, &portEvent, &timeout)) {  // wie inotify-wait
            ...
            if (portEvent.portev_source != PORT_SOURCE_FILE) {
                LogError(errno, RS_RET_SYS_ERR,
                         "do_fen: Event from unexpected source "
                         ": %d\n",
            ...
            DBGPRINTF("do_fen event received: deleted %d, is_file %d, name '%s' foname '%s'\n", act->is_deleted,
                      act->edge->is_file, act->name, ((struct file_obj *)portEvent.portev_object)->fo_name);
            if (act->is_deleted) {
                free(act->name);
                free(act);
                continue;
...
static rsRetVal do_fen(void) {
    LogError(0, RS_RET_NOT_IMPLEMENTED,
             "do_fen: mode set to fen, but the "
...
BEGINrunInput
...
#endif
    DBGPRINTF("working in %s mode\n", (runModConf->opMode == OPMODE_POLLING)
                                          ? "polling"
    ...
    if (runModConf->opMode == OPMODE_POLLING)
        ...
        LogError(0, RS_RET_NOT_IMPLEMENTED, "imfile: unknown mode %d set", runModConf->opMode);
        return RS_RET_NOT_IMPLEMENTED;
    }
 ...
 * jvymazal 2019-11-27
 */
static void removeOldStatefile(const uchar *statefn, const char *hashToDelete) {
    int ret;
    ...
    if (ret != 0) {
        if (errno != ENOENT) {
            ...
            DBGPRINTF(
                "trying to delete no longer valid statefile '%s' which no "
 ...
 */
static rsRetVal ATTR_NONNULL() persistStrmState(act_obj_t *const act) {
    DEFiRet;
    uchar statefile[MAXFNAME];
    uchar statefname[MAXFNAME];
...
static inline void std_checkRuleset_genErrMsg(__attribute__((unused)) modConfData_t *modConf, instanceConf_t *inst) {
    LogError(0, NO_ERRCODE,
             "imfile: ruleset '%s' for %s not found - "
             "using default ruleset instead",
             inst->pszBindRuleset, inst->pszFileName);
}
...
BEGINmodInit()
    ...
    *ipIFVersProvided = CURR_MOD_IF_VERSION; /* we only support the current interface specification */
    ...
    CHKiRet(
        omsdRegCFSLineHdlr((uchar *)"inputfiletag", 0, eCmdHdlrGetWord, NULL, &cs.pszFileTag, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputfilestatefile", 0, eCmdHdlrGetWord, NULL, &cs.pszStateFile,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputfileseverity", 0, eCmdHdlrSeverity, NULL, &cs.iSeverity,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputfilefacility", 0, eCmdHdlrFacility, NULL, &cs.iFacility,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(
        omsdRegCFSLineHdlr((uchar *)"inputfilereadmode", 0, eCmdHdlrInt, NULL, &cs.readMode, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputfilemaxlinesatonce", 0, eCmdHdlrSize, NULL, &cs.maxLinesAtOnce,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputfiletrimlineoverbytes", 0, eCmdHdlrSize, NULL, &cs.trimLineOverBytes,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputfilepersiststateinterval", 0, eCmdHdlrInt, NULL,
                               &cs.iPersistStateInterval, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputfilebindruleset", 0, eCmdHdlrGetWord, NULL, &cs.pszBindRuleset,
                               STD_LOADABLE_MODULE_ID));
    /* that command ads a new file! */
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputrunfilemonitor", 0, eCmdHdlrGetWord, addInstance, NULL,
                               STD_LOADABLE_MODULE_ID));
    /* module-global config params - will be disabled in configs that are loaded
     * via module(...).
     */
    CHKiRet(regCfSysLineHdlr2((uchar *)"inputfilepollinterval", 0, eCmdHdlrInt, NULL, &cs.iPollInterval,

...
do_fen(void)
{
	...
	for(pLstn = runModConf->pRootLstn ; pLstn != NULL ; pLstn = pLstn->next) {
		if(pLstn->masterLstn == NULL) {
			...
			pLstn->pfinf->events = FILE_MODIFIED; // |FILE_DELETE|FILE_RENAME_TO|FILE_RENAME_FROM;
	...
	DBGPRINTF("imfile: do_fen: ENTER monitoring loop \n");
	while(glbl.GetGlobalInputTermState() == 0) {
		DBGPRINTF("imfile: do_fen: loop begin... \n");
		/* Loop through events, if there are any */
		while (!port_get(port, &portEvent, &timeout)) {
			/*
			* Can add cases for other sources if this
			* port is used to collect events from multiple sources.
			*/
			DBGPRINTF("imfile: do_fen: event received, processing ... \n");
	...
	DBGPRINTF("imfile: do_fen: EXIT monitoring loop \n");
...
do_fen(void)
{
	errmsg.LogError(0, RS_RET_NOT_IMPLEMENTED, "imfile: mode set to fen, but the "

...
static void skipWhitespace(uchar **buf) {
    while (**buf && isspace(**buf)) ++(*buf);
}


static rsRetVal parseName(uchar **buf, char *name, unsigned lenName) {
    unsigned i;
    skipWhitespace(buf);
...
static rsRetVal parseValue(uchar **buf, char *val, unsigned lenval) {
    char termc;
    unsigned i;
    DEFiRet;
 ...
 */
static rsRetVal audit_parse(uchar *buf, struct json_object **jsonRoot) {
...
BEGINdoAction_NoStrings
    ...
    if (jsonRoot == NULL) {
        DBGPRINTF(
            "mmaudit: audit parse error, assuming no "
            "audit message: '%s'\n",
            buf);
        FINALIZE;
    }
...
BEGINmodInit()
    ...
    *ipIFVersProvided = CURR_MOD_IF_VERSION;
    ...
    if (!bMsgPassingSupported) {
        DBGPRINTF(
            "mmaudit: msg-passing is not supported by rsyslog core, "
            "can not continue.\n");

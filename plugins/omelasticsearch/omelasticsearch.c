...
curlResult(void *ptr, size_t size, size_t nmemb, void *userdata)
{
	...
	pWrkrData->reply = buf;
DBGPRINTF("curlResult: wrkrData %p, add %zd, new size %d, new ptr %p\n", pWrkrData, size*nmemb, pWrkrData->replyLen, pWrkrData->reply);
	return size*nmemb;
}
...
finalize_it:
	if(urlBuf != NULL)
		es_deleteStr(urlBuf);
DBGPRINTF("checkConn free reply %p, size was %d\n", pWrkrData->reply, (int) pWrkrData->replyLen);

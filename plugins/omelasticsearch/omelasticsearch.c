...
BEGINfreeWrkrInstance
	CODESTARTfreeWrkrInstance if (pWrkrData->curlHeader != NULL)
	{
...
#define SCHEME_HTTP "http://"
	...
	*/
	if (strcasestr(serverParam, SCHEME_HTTP))
		...
		r = useHttps ? es_addBuf(&urlBuf, SCHEME_HTTPS, sizeof(SCHEME_HTTPS) - 1) : es_addBuf(&urlBuf, SCHEME_HTTP, sizeof(SCHEME_HTTP) - 1);

	if (r == 0)
		r = es_addBuf(&urlBuf, serverParam, strlen(serverParam));
	if (r == 0 && !strchr(host, ':')) {
	...
	if (r == 0)
		r = es_addChar(&urlBuf, '/');
	if (r == 0)
		*baseUrl = (uchar *)es_str2cstr(urlBuf, NULL);
...
#define HEALTH_URI "_cat/health"
	...
	for (i = 0; i < pWrkrData->pData->numServers; ++i) {
		...
		if (r == 0)
			r = es_addBuf(&urlBuf, HEALTH_URI, sizeof(HEALTH_URI) - 1);
		if (r == 0)
...
setPostURL(wrkrInstanceData_t *pWrkrData, instanceData *pData, uchar **tpls)
{
	...
	if (bulkmode) {
		...
		if (r == 0)
			r = es_addChar(&url, '/');
		if (r == 0)
			r = es_addBuf(&url, (char *)searchType, ustrlen(searchType));
	}
	...
	if (pData->timeout != NULL) {
		if (r == 0)
			r = es_addChar(&url, separator);
		if (r == 0)
			r = es_addBuf(&url, "timeout=", sizeof("timeout=") - 1);
		if (r == 0)
			r = es_addBuf(&url, (char *)pData->timeout, ustrlen(pData->timeout));
		separator = '&';
	}
	...
	if (parent != NULL) {
		if (r == 0)
			r = es_addChar(&url, separator);
		if (r == 0)
			r = es_addBuf(&url, "parent=", sizeof("parent=") - 1);
		if (r == 0)
			es_addBuf(&url, (char *)parent, ustrlen(parent));
	}
...
buildBatch(wrkrInstanceData_t *pWrkrData, uchar *message, uchar **tpls)
{
	...
	if (r == 0)
		r = es_addBuf(&pWrkrData->batch.data, (char *)searchIndex,
		    ustrlen(searchIndex));
	if (r == 0)
		r = es_addBuf(&pWrkrData->batch.data, META_TYPE, sizeof(META_TYPE) - 1);
	if (r == 0)
		r = es_addBuf(&pWrkrData->batch.data, (char *)searchType,
		    ustrlen(searchType));
	if (parent != NULL) {
		if (r == 0)
			r = es_addBuf(&pWrkrData->batch.data, META_PARENT, sizeof(META_PARENT) - 1);
		if (r == 0)
			r = es_addBuf(&pWrkrData->batch.data, (char *)parent, ustrlen(parent));
	}
	if (bulkId != NULL) {
		if (r == 0)
			r = es_addBuf(&pWrkrData->batch.data, META_ID, sizeof(META_ID) - 1);
		if (r == 0)
			r = es_addBuf(&pWrkrData->batch.data, (char *)bulkId, ustrlen(bulkId));
	}
	if (r == 0)
		r = es_addBuf(&pWrkrData->batch.data, META_END, sizeof(META_END) - 1);
	if (r == 0)
		r = es_addBuf(&pWrkrData->batch.data, (char *)message, length);
	if (r == 0)
		r = es_addBuf(&pWrkrData->batch.data, "\n", sizeof("\n") - 1);
	if (r != 0) {
...
getDataErrorDefault(wrkrInstanceData_t *pWrkrData, cJSON **pReplyRoot, uchar *reqmsg, char **rendered)
{
	...
	if ((req = cJSON_CreateObject()) == NULL)
		ABORT_FINALIZE(RS_RET_ERR);
...
curlPost(wrkrInstanceData_t *pWrkrData, uchar *message, int msglen, uchar **tpls, int nmsgs)
{
	...
	if (code == CURLE_COULDNT_RESOLVE_HOST || code == CURLE_COULDNT_RESOLVE_PROXY || code == CURLE_COULDNT_CONNECT || code == CURLE_WRITE_ERROR) {
...
BEGINbeginTransaction
	CODESTARTbeginTransaction if (!pWrkrData->pData->bulkmode)
	{
		FINALIZE;
	}
...
BEGINparseSelectorAct
	CODESTARTparseSelectorAct
	    CODE_STD_STRING_REQUESTparseSelectorAct(1) if (!strncmp((char *)p, ":omelasticsearch:", sizeof(":omelasticsearch:") - 1))
	{
...
BEGINdoHUP
	CODESTARTdoHUP if (pData->fdErrFile != -1)
	{
		close(pData->fdErrFile);
		pData->fdErrFile = -1;
	}

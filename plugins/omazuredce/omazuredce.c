...
static rsRetVal postBatchToAzure(instanceData *pData, wrkrInstanceData_t *pWrkrData, size_t payloadLen) {
...

    {
        ...
        if (httpCode == 408 || httpCode == 429 || httpCode >= 500) {
            LogError(0, RS_RET_SUSPENDED, "omazuredce: batch post HTTP status=%ld response='%s' (will retry)", httpCode,
                     response.data == NULL ? "" : response.data);
            ABORT_FINALIZE(RS_RET_SUSPENDED);
        }
...
BEGINnewActInst
    ...
    CHKmalloc(tplToUse = (uchar *)strdup((pData->templateName == NULL) ? " StdJSONFmt" : (char *)pData->templateName));

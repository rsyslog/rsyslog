...
static rsRetVal omsnmp_sendsnmp(wrkrInstanceData_t *pWrkrData, uchar *psz)
{
	...
	/* If SNMP Version2c is configured !*/
		...
		if (snmp_add_var(pdu, objid_snmptrap, sizeof(objid_snmptrap) / sizeof(oid), 'o',
			pData->szSnmpTrapOID == NULL ? "1.3.6.1.4.1.19406.1.2.1" : (char *)pData->szSnmpTrapOID) != 0) {
			strErr = snmp_api_errstring(snmp_errno);
	...
	if (snmp_parse_oid(pData->szSyslogMessageOID == NULL ? "1.3.6.1.4.1.19406.1.1.2.1" : (char *)pData->szSyslogMessageOID,
...
BEGINnewActInst
	...
	CHKiRet(OMSRsetEntry(*ppOMSR, 0, (uchar *)strdup((pData->tplName == NULL) ? "RSYSLOG_FileFormat" : (char *)pData->tplName),

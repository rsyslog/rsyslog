...
DEFobjCurrIf(errmsg) static int bDbiInitialized = 0; /* dbi_initialize() can only be called one - this keeps track of it */
...
BEGINtryResume
	CODESTARTtryResume if (pWrkrData->pData->conn == NULL)
	{
...
BEGINbeginTransaction
	CODESTARTbeginTransaction if (pWrkrData->pData->conn == NULL)
	{
...
BEGINparseSelectorAct
	...
	if ((pData->drvrName = (uchar *)strdup((char *)cs.drvrName)) == NULL)

...
static struct cnfparamdescr actpdescr[] = {
	...
	{"region",         eCmdHdlrString,  0},
	{"bearer_token",   eCmdHdlrString,  0},
	{"log_group",      eCmdHdlrString,  0},
	{"log_stream",     eCmdHdlrString,  0},
	{"max_batch_size", eCmdHdlrInt,     0},
};
...
static rsRetVal appendEvent(wrkrInstanceData_t *pWrkrData, const char *msg) {
...

	{
		...
		for (size_t i = 0; i < msgLen && j < escapedMaxLen - 1; i++) {
			...
			switch (c) {
			case '"':  escapedMsg[j++] = '\\'; escapedMsg[j++] = '"'; break;
			case '\\': escapedMsg[j++] = '\\'; escapedMsg[j++] = '\\'; break;
			case '\b': escapedMsg[j++] = '\\'; escapedMsg[j++] = 'b'; break;
			case '\f': escapedMsg[j++] = '\\'; escapedMsg[j++] = 'f'; break;
			case '\n': escapedMsg[j++] = '\\'; escapedMsg[j++] = 'n'; break;
			case '\r': escapedMsg[j++] = '\\'; escapedMsg[j++] = 'r'; break;
			case '\t': escapedMsg[j++] = '\\'; escapedMsg[j++] = 't'; break;
			default:
				if (c < 0x20) {
	...
	size_t fullLen = strlen("{\"event\":\"\",\"time\":,\"host\":\"\",\"source\":\"rsyslog\"}")
	                 + strlen(escapedMsg) + 32 /* timestamp */ + strlen(hostname) + 1;
	...
	int n = snprintf(fullEvent, fullLen,
		"{\"event\":\"%s\",\"time\":%.6f,\"host\":\"%s\",\"source\":\"rsyslog\"}",
	...
	if (needed > CWL_MAX_REQUEST_BYTES) {
		...
		if (needed > CWL_MAX_REQUEST_BYTES) {
			...
			LogError(0, RS_RET_ERR,
				"omawslogshlc: single event size %zu exceeds 1 MiB limit",
				eventLen);
			ABORT_FINALIZE(RS_RET_ERR);
		}
	}
...
static rsRetVal postBatch(wrkrInstanceData_t *pWrkrData) {
...

	{
		...
		n = snprintf(url, urlLen,
			"https://logs.%s.amazonaws.com/services/collector/event?logGroup=%s&logStream=%s",
	...
	if (curlRes != CURLE_OK) {
		LogError(0, RS_RET_SUSPENDED,
			"omawslogshlc: HTTP POST failed: %s", curl_easy_strerror(curlRes));
	...
	if (httpCode == 429 || httpCode >= 500) {
		LogError(0, RS_RET_SUSPENDED,
			"omawslogshlc: HTTP status=%ld (will retry)", httpCode);
		ABORT_FINALIZE(RS_RET_SUSPENDED);
	}
	...
	if (httpCode == 401 || httpCode == 403) {
		LogError(0, RS_RET_SUSPENDED,
			"omawslogshlc: HTTP status=%ld — check bearer_token configuration", httpCode);
		ABORT_FINALIZE(RS_RET_SUSPENDED);
	}

	/* Non-retryable client errors */
	LogError(0, RS_RET_ERR,
		"omawslogshlc: HTTP status=%ld (non-retryable)", httpCode);
	resetBatch(pWrkrData);
	ABORT_FINALIZE(RS_RET_ERR);
...
BEGINnewActInst
	...
	DBGPRINTF("omawslogshlc: config region='%s' log_group='%s' log_stream='%s' "
		"max_batch_size=%d bearer_token=%s\n",
		safeStr(pData->region), safeStr(pData->logGroup), safeStr(pData->logStream),
		pData->maxBatchSize, pData->bearerToken == NULL ? "<unset>" : "<set>");

	/* Validate required parameters */
	if (pData->region == NULL || pData->region[0] == '\0') {
		LogError(0, RS_RET_PARAM_ERROR,
			"omawslogshlc: parameter 'region' is required");
		ABORT_FINALIZE(RS_RET_PARAM_ERROR);
	}
	if (pData->bearerToken == NULL || pData->bearerToken[0] == '\0') {
		LogError(0, RS_RET_PARAM_ERROR,
			"omawslogshlc: parameter 'bearer_token' is required");
		ABORT_FINALIZE(RS_RET_PARAM_ERROR);
	}
	if (pData->logGroup == NULL || pData->logGroup[0] == '\0') {
		LogError(0, RS_RET_PARAM_ERROR,
			"omawslogshlc: parameter 'log_group' is required");
		ABORT_FINALIZE(RS_RET_PARAM_ERROR);
	}
	if (pData->logStream == NULL || pData->logStream[0] == '\0') {
		LogError(0, RS_RET_PARAM_ERROR,
			"omawslogshlc: parameter 'log_stream' is required");
		ABORT_FINALIZE(RS_RET_PARAM_ERROR);
	}
	if (pData->maxBatchSize <= 0 || pData->maxBatchSize > CWL_MAX_EVENTS_PER_REQUEST) {
		LogError(0, RS_RET_PARAM_ERROR,
			"omawslogshlc: max_batch_size must be 1..%d, got %d",
	...
	CHKmalloc(tplToUse = (uchar *)strdup(
		(pData->tplName == NULL) ? "RSYSLOG_TraditionalFileFormat" : (char *)pData->tplName));
	...
	*ipIFVersProvided = CURR_MOD_IF_VERSION;
	...
	if (curl_global_init(CURL_GLOBAL_ALL) != 0) {
		LogError(0, RS_RET_OBJ_CREATION_FAILED,
			"omawslogshlc: curl_global_init failed");

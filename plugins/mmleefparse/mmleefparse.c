...
static struct cnfparamdescr actpdescr[] = {{"cookie", eCmdHdlrString, 0},
                                           {"container", eCmdHdlrString, 0},
                                           {"userawmsg", eCmdHdlrBinary, 0},
                                           {"delimiter", eCmdHdlrString, 0},
                                           {"searchcookie", eCmdHdlrBinary, 0},
                                           {"searchwindow", eCmdHdlrInt, 0}};
...
static ATTR_NONNULL() rsRetVal processLEEF(wrkrInstanceData_t *pWrkrData, smsg_t *pMsg, const char *payload) {
    ...
    CHKiRet(parse_leef_header(payload, &protocolVersion, &vendor, &product, &productVersion, &eventID, &extensionStart));
...
BEGINdoAction_NoStrings
    ...
    sbool haveCookie = 0;
    if (pData->bUseRawMsg) {
        if (len >= (rs_size_t)pData->lenCookie &&
            memcmp(buf, pData->cookie, (size_t)pData->lenCookie) == 0) {
            haveCookie = 1;
        }
    } else {

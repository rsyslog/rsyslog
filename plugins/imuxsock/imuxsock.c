...
DEFobjCurrIf(glbl) DEFobjCurrIf(prop) DEFobjCurrIf(net) DEFobjCurrIf(parser) DEFobjCurrIf(datetime)
    DEFobjCurrIf(statsobj) DEFobjCurrIf(ruleset)
...
struct modConfData_s {
    ...
    unsigned int ratelimitBurstSysSock;
    int ratelimitSeveritySysSock;
    ratelimit_config_t *ratelimitCfgSysSock;
...
static struct cnfparamdescr modpdescr[] = {{"syssock.use", eCmdHdlrBinary, 0},
...
static rsRetVal addInstance(void __attribute__((unused)) * pVal, uchar *pNewVal) {
    ...
    if (pNewVal == NULL || pNewVal[0] == '\0') {
        LogError(0, RS_RET_SOCKNAME_MISSING,
                 "imuxsock: socket name must be specified, "
                 "but is not - listener not created\n");
        if (pNewVal != NULL) free(pNewVal);
...
static rsRetVal addListner(instanceConf_t *inst) {
    ...
    listeners[nfd].ratelimitCfg = inst->ratelimitCfg;
    ...
    if (listeners[nfd].ratelimitCfg != NULL) {
        const char *const rlName = (listeners[nfd].sockName == NULL) ? NULL : (char *)listeners[nfd].sockName;
        CHKiRet(ratelimitNewFromConfig(&listeners[nfd].dflt_ratelimiter, listeners[nfd].ratelimitCfg, rlName));
    } else {
...
static rsRetVal
    ...
    if (pLstn->bUnlink) unlink((char *)pLstn->sockName);
...
static rsRetVal openLogSocket(lstn_t *pLstn) {
    DEFiRet;
#ifdef HAVE_SCM_CREDENTIALS
    int one;
#endif /* HAVE_SCM_CREDENTIALS */

    if (pLstn->sockName[0] == '\0') return -1;
    ...
    if (sd_fds > 0) {
        ...
        for (fd = SD_LISTEN_FDS_START; fd < SD_LISTEN_FDS_START + sd_fds; fd++) {
            if (sd_is_socket_unix(fd, SOCK_DGRAM, -1, (const char *)pLstn->sockName, 0) == 1) {
                ...
                LogMsg(0, NO_ERRCODE, LOG_INFO, "imuxsock: Acquired UNIX socket '%s' (fd %d) from systemd.\n",
 ...
 */
static rsRetVal findRatelimiter(lstn_t *pLstn, struct ucred *cred, ratelimit_t **prl) {
    ratelimit_t *rl = NULL;
    int r;
    pid_t *keybuf;
    char pinfobuf[512];
    ...
    if (rl == NULL) {
        ...
        if (pLstn->ratelimitCfg != NULL) {
            CHKiRet(ratelimitNewFromConfig(&rl, pLstn->ratelimitCfg, pinfobuf));
        } else {
 ...
 */
static void fixPID(uchar *bufTAG, int *lenTag, struct ucred *cred) {
    int i;
    char bufPID[16];
    int lenPID;
    ...
    for (i = *lenTag; i >= 0 && bufTAG[i] != '['; --i) /*JUST SKIP*/
        ;

    if (i < 0) i = *lenTag - 1; /* go right at end of TAG, pid was not present (-1 for ':') */

    if (i + lenPID > CONF_TAG_MAXSIZE) return; /* do not touch, as things would break */
...
static rsRetVal getTrustedProp(struct ucred *cred, const char *propName, uchar *buf, size_t lenBuf, int *lenProp) {
    ...
    if (snprintf(namebuf, sizeof(namebuf), "/proc/%lu/%s", (long unsigned)cred->pid, propName) >=
        (int)sizeof(namebuf)) {
 ...
 */
static rsRetVal getTrustedExe(struct ucred *cred, uchar *buf, size_t lenBuf, int *lenProp) {
    int lenRead;
    char namebuf[1024];
    DEFiRet;

    if (snprintf(namebuf, sizeof(namebuf), "/proc/%lu/exe", (long unsigned)cred->pid) >= (int)sizeof(namebuf)) {
        ABORT_FINALIZE(RS_RET_ERR);
    }
 ...
 * characters added.
 */
static int copyescaped(uchar *dstbuf, uchar *inbuf, int inlen) {
    int iDst, iSrc;
 ...
 */
static rsRetVal SubmitMsg(uchar *pRcv, int lenRcv, lstn_t *pLstn, struct ucred *cred, struct timeval *ts) {
    smsg_t *pMsg = NULL;
    int lenMsg;
    int offs;
    int i;
...
#undef CHKjson
    ...
    if (pLstn->bUseSpecialParser) {
        ...
        if (ts == NULL) {
            if ((pLstn->flags & IGNDATE)) {
                ...
                if (datetime.ParseTIMESTAMP3339(&(pMsg->tTIMESTAMP), &parse, &lenMsg) != RS_RET_OK &&
                    datetime.ParseTIMESTAMP3164(&(pMsg->tTIMESTAMP), &parse, &lenMsg, NO_PARSE3164_TZSTRING,
        ...
        if (pLstn->bWritePid) fixPID(bufParseTAG, &i, cred);
...
/*  AIXPORT : MSG_DONTWAIT not supported */
    ...
    #ifdef HAVE_SCM_CREDENTIALS
                if (pLstn->bUseCreds && cm->cmsg_level == SOL_SOCKET && cm->cmsg_type == SCM_CREDENTIALS) {
                    memcpy(&cred, CMSG_DATA(cm), sizeof(cred));
                    cred_set = 1;
                }
    #endif /* HAVE_SCM_CREDENTIALS */
    #if HAVE_SO_TIMESTAMP
                if (pLstn->bUseSysTimeStamp && cm->cmsg_level == SOL_SOCKET && cm->cmsg_type == SO_TIMESTAMP) {
...
finalize_it:
    if (pRcv != NULL && (size_t)iMaxLine >= sizeof(bufRcv) - 1) free(pRcv);
...
static rsRetVal activateListeners(void) {
    ...
    if (startIndexUxLocalSockets == 0) {
        ...
        listeners[0].ratelimitCfg = runModConf->ratelimitCfgSysSock;
        ...
        if (listeners[0].ratelimitCfg != NULL) {
            CHKiRet(ratelimitNewFromConfig(&listeners[0].dflt_ratelimiter, listeners[0].ratelimitCfg,
                                           (char *)listeners[0].sockName));
        } else {
    ...
    if (actSocks == 0) {
        LogError(0, RS_RET_ERR,
                 "imuxsock does not run because we could not "
                 "acquire any socket\n");
        ABORT_FINALIZE(RS_RET_ERR);
    }
...
BEGINbeginCnfLoad
    ...
    pModConf->ratelimitCfgSysSock = NULL;
...
BEGINsetModCnf
    ...
    int i;
    char *syssockRlName = NULL;
    sbool syssockRlParamsUsed = 0;
    CODESTARTsetModCnf;
    ...
    for (i = 0; i < modpblk.nParams; ++i) {
        ...
        if (!strcmp(modpblk.descr[i].name, "syssock.use")) {
            ...
            free(syssockRlName);
            syssockRlName = es_str2cstr(pvals[i].val.d.estr, NULL);
    ...
    const sbool needSysSockRlCfg =
        (loadModConf->ratelimitIntervalSysSock > 0) || syssockRlParamsUsed || (syssockRlName != NULL);
    if (needSysSockRlCfg) {
        unsigned int interval = loadModConf->ratelimitIntervalSysSock;
        unsigned int burst = loadModConf->ratelimitBurstSysSock;
        int severity = loadModConf->ratelimitSeveritySysSock;
        const rsRetVal rlRet =
            ratelimitResolveFromValues(loadModConf->pConf, "syssock", syssockRlName, syssockRlParamsUsed, &interval,
                                       &burst, &severity, &loadModConf->ratelimitCfgSysSock);
        if (rlRet == RS_RET_OK) {
            loadModConf->ratelimitIntervalSysSock = interval;
            loadModConf->ratelimitBurstSysSock = burst;
            loadModConf->ratelimitSeveritySysSock = severity;
        } else {
            LogError(0, rlRet, "imuxsock: unable to resolve syssock ratelimit configuration");
        }
    }
...
BEGINnewInpInst
    ...
    if (pvals == NULL) {
        LogError(0, RS_RET_MISSING_CNFPARAMS, "imuxsock: required parameter are missing\n");
        ABORT_FINALIZE(RS_RET_MISSING_CNFPARAMS);
    }
    ...
    if (iRet == RS_RET_OK) {
        if (inst->ratelimitInterval > 0 || ratelimitParamsUsed || ratelimitName != NULL) {
            unsigned int interval = inst->ratelimitInterval;
            unsigned int burst = inst->ratelimitBurst;
            int severity = inst->ratelimitSeverity;
            const rsRetVal rlRet =
                ratelimitResolveFromValues(loadModConf->pConf, "imuxsock", ratelimitName, ratelimitParamsUsed,
                                           &interval, &burst, &severity, &inst->ratelimitCfg);
            if (rlRet == RS_RET_OK) {
                inst->ratelimitInterval = interval;
                inst->ratelimitBurst = burst;
                inst->ratelimitSeverity = severity;
            } else {
                LogError(0, rlRet, "imuxsock: unable to resolve ratelimit configuration for socket %s",
                         inst->sockName == NULL ? "(unset)" : (char *)inst->sockName);
            }
        }
    }
...
BEGINendCnfLoad
    ...
    if (!loadModConf->configSetViaV2Method) {
        ...
        if (loadModConf->ratelimitCfgSysSock == NULL) {
            unsigned int interval = loadModConf->ratelimitIntervalSysSock;
            unsigned int burst = loadModConf->ratelimitBurstSysSock;
            int severity = loadModConf->ratelimitSeveritySysSock;
            const rsRetVal rlRet = ratelimitResolveFromValues(loadModConf->pConf, "syssock", NULL, 0, &interval, &burst,
                                                              &severity, &loadModConf->ratelimitCfgSysSock);
            if (rlRet == RS_RET_OK) {
                loadModConf->ratelimitIntervalSysSock = interval;
                loadModConf->ratelimitBurstSysSock = burst;
                loadModConf->ratelimitSeveritySysSock = severity;
            } else {
                LogError(0, rlRet, "imuxsock: unable to resolve legacy syssock ratelimit configuration");
            }
        }
    }
...
static void std_checkRuleset_genErrMsg(__attribute__((unused)) modConfData_t *modConf, instanceConf_t *inst) {
    LogError(0, NO_ERRCODE,
             "imuxsock: ruleset '%s' for socket %s not found - "
             "using default ruleset instead",
             inst->pszBindRuleset, inst->sockName);
}
BEGINcheckCnf
    instanceConf_t *inst;
    CODESTARTcheckCnf;
...
BEGINafterRun
    ...
    for (i = 0; i < nfd; i++)
        if (listeners[i].fd != -1) close(listeners[i].fd);
    ...
    for (i = startIndexUxLocalSockets; i < nfd; i++)
        if (listeners[i].sockName && listeners[i].fd != -1) {
            ...
            if (sd_fds > 0
#ifdef HAVE_LIBSYSTEMD
                && listeners[i].fd >= SD_LISTEN_FDS_START && listeners[i].fd < SD_LISTEN_FDS_START + sd_fds
#endif
            )
                continue;
...
BEGINmodInit()
    ...
    *ipIFVersProvided = CURR_MOD_IF_VERSION; /* we only support the current interface specification */
    ...
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputunixlistensocketignoremsgtimestamp", 0, eCmdHdlrBinary, NULL,
                               &cs.bIgnoreTimestamp, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputunixlistensockethostname", 0, eCmdHdlrGetWord, NULL, &cs.pLogHostName,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputunixlistensocketflowcontrol", 0, eCmdHdlrBinary, NULL, &cs.bUseFlowCtl,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputunixlistensocketannotate", 0, eCmdHdlrBinary, NULL, &cs.bAnnotate,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputunixlistensocketcreatepath", 0, eCmdHdlrBinary, NULL, &cs.bCreatePath,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputunixlistensocketusesystimestamp", 0, eCmdHdlrBinary, NULL,
                               &cs.bUseSysTimeStamp, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"addunixlistensocket", 0, eCmdHdlrGetWord, addInstance, NULL,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputunixlistensocketusepidfromsystem", 0, eCmdHdlrBinary, NULL, &cs.bWritePid,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"imuxsockratelimitinterval", 0, eCmdHdlrInt, NULL, &cs.ratelimitInterval,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"imuxsockratelimitburst", 0, eCmdHdlrInt, NULL, &cs.ratelimitBurst,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"imuxsockratelimitseverity", 0, eCmdHdlrInt, NULL, &cs.ratelimitSeverity,
                               STD_LOADABLE_MODULE_ID));
     ...
     */
    CHKiRet(regCfSysLineHdlr2((uchar *)"omitlocallogging", 0, eCmdHdlrBinary, NULL, &cs.bOmitLocalLogging,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogsocketname", 0, eCmdHdlrGetWord, NULL, &cs.pLogSockName,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogsocketignoremsgtimestamp", 0, eCmdHdlrBinary, NULL,
                              &cs.bIgnoreTimestampSysSock, STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogsocketflowcontrol", 0, eCmdHdlrBinary, NULL, &cs.bUseFlowCtlSysSock,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogusesystimestamp", 0, eCmdHdlrBinary, NULL, &cs.bUseSysTimeStampSysSock,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogsocketannotate", 0, eCmdHdlrBinary, NULL, &cs.bAnnotateSysSock,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogparsetrusted", 0, eCmdHdlrBinary, NULL, &cs.bParseTrusted,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogusepidfromsystem", 0, eCmdHdlrBinary, NULL, &cs.bWritePidSysSock,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogratelimitinterval", 0, eCmdHdlrInt, NULL, &cs.ratelimitIntervalSysSock,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogratelimitburst", 0, eCmdHdlrInt, NULL, &cs.ratelimitBurstSysSock,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"systemlogratelimitseverity", 0, eCmdHdlrInt, NULL, &cs.ratelimitSeveritySysSock,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    ...
    CHKiRet(
        statsobj.AddCounter(modStats, UCHAR_CONSTANT("submitted"), ctrType_IntCtr, CTR_FLAG_RESETTABLE, &ctrSubmit));
    ...
    CHKiRet(statsobj.AddCounter(modStats, UCHAR_CONSTANT("ratelimit.numratelimiters"), ctrType_IntCtr,
                                CTR_FLAG_RESETTABLE, &ctrNumRatelimiters));

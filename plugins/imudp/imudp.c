...
static struct cnfparamdescr modpdescr[] = {{"schedulingpolicy", eCmdHdlrGetWord, 0},
...
static rsRetVal addInstance(void __attribute__((unused)) * pVal, uchar *pNewVal) {
    ...
    CHKmalloc(inst->pszBindPort = ustrdup((pNewVal == NULL || *pNewVal == '\0') ? (uchar *)"514" : pNewVal));
...
static rsRetVal addListner(instanceConf_t *inst) {
    ...
    if (newSocks != NULL) {
        ...
        for (iSrc = 1; iSrc <= newSocks[0]; ++iSrc) {
            ...
            CHKiRet(prop.SetString(newlcnfinfo->pInputName, inputname, ustrlen(inputname)));
            ...
            CHKiRet(statsobj.AddCounter(newlcnfinfo->stats, UCHAR_CONSTANT("submitted"), ctrType_IntCtr,
                                        CTR_FLAG_RESETTABLE, &(newlcnfinfo->ctrSubmit)));
             ...
             * conflicting matches.
             */
            if (lcnfRoot == NULL) lcnfRoot = newlcnfinfo;
            if (lcnfLast == NULL)
...
finalize_it:
    if (iRet != RS_RET_OK) {
        if (newlcnfinfo != NULL) {
            if (newlcnfinfo->ratelimiter != NULL) ratelimitDestruct(newlcnfinfo->ratelimiter);
            if (newlcnfinfo->pInputName != NULL) prop.Destruct(&newlcnfinfo->pInputName);
            if (newlcnfinfo->stats != NULL) statsobj.Destruct(&newlcnfinfo->stats);
            free(newlcnfinfo);
        }
...
static inline void std_checkRuleset_genErrMsg(__attribute__((unused)) modConfData_t *modConf, instanceConf_t *inst) {
    LogError(0, NO_ERRCODE,
             "imudp: ruleset '%s' for %s:%s not found - "
             "using default ruleset instead",
             inst->pszBindRuleset, inst->pszBindAddr == NULL ? "*" : (char *)inst->pszBindAddr, inst->pszBindPort);
}
...
static rsRetVal processPacket(struct lstn_s *lstn,
    ...
    if (lenRcvBuf == 0) FINALIZE; /* this looks a bit strange, but practice shows it happens... */
            ...
            *pbIsPermitted = net.isAllowedSender2((uchar *)"UDP", (struct sockaddr *)frominet, "", 0);
            ...
            if (*pbIsPermitted == 0) {
                ...
                if (glbl.GetOptionDisallowWarning(runModConf->pConf)) {
                    LogError(0, NO_ERRCODE, "imudp: UDP message from disallowed sender discarded");
                }
            }
        }
    } else {
    ...
    if (*pbIsPermitted != 0) {
        ...
        if (lstn->dfltTZ != NULL) MsgSetDfltTZ(pMsg, (char *)lstn->dfltTZ);
 ...
 */
static rsRetVal checkSchedulingPriority(modConfData_t *modConf) {
    DEFiRet;
    ...
    if (modConf->iSchedPrio < sched_get_priority_min(modConf->iSchedPolicy) ||
        ...
        LogError(0, NO_ERRCODE,
                 ...
                 modConf->iSchedPrio, sched_get_priority_min(modConf->iSchedPolicy),
                 sched_get_priority_max(modConf->iSchedPolicy), modConf->pszSchedPolicy);
 ...
 */
static rsRetVal checkSchedulingPolicy(modConfData_t *modConf) {
    DEFiRet;
...
static rsRetVal checkSchedParam(modConfData_t *modConf) {
    DEFiRet;

    if (modConf->pszSchedPolicy != NULL && modConf->iSchedPrio == SCHED_PRIO_UNSET) {
        LogError(0, RS_RET_ERR_SCHED_PARAMS, "imudp: scheduling policy set, but without priority - ignoring settings");
        ABORT_FINALIZE(RS_RET_ERR_SCHED_PARAMS);
    } else if (modConf->pszSchedPolicy == NULL && modConf->iSchedPrio != SCHED_PRIO_UNSET) {
        LogError(0, RS_RET_ERR_SCHED_PARAMS, "imudp: scheduling priority set, but without policy - ignoring settings");
        ABORT_FINALIZE(RS_RET_ERR_SCHED_PARAMS);
    } else if (modConf->pszSchedPolicy != NULL && modConf->iSchedPrio != SCHED_PRIO_UNSET) {
...
finalize_it:
    if (iRet != RS_RET_OK) modConf->iSchedPrio = SCHED_PRIO_UNSET; /* prevents doing the activation call */

    RETiRet;
}

/* set the configured scheduling policy (if possible) */
static rsRetVal setSchedParams(modConfData_t *modConf) {
    DEFiRet;
    ...
    if (modConf->iSchedPrio == SCHED_PRIO_UNSET) FINALIZE;
...
static rsRetVal rcvMainLoop(struct wrkrInfo_s *const __restrict__ pWrkr) {
    ...
    nLstn = 0;
    for (lstn = lcnfRoot; lstn != NULL; lstn = lstn->next) ++nLstn;
...
finalize_it:
    if (udpEPollEvt != NULL) free(udpEPollEvt);

    RETiRet;
}
#else /* #if HAVE_EPOLL_CREATE1 */
/* this is the code for the select() interface */
static rsRetVal ATTR_NONNULL() rcvMainLoop(struct wrkrInfo_s *const __restrict__ pWrkr) {
    DEFiRet;
    int nfds;
    struct sockaddr_storage frominetPrev;
    int bIsPermitted;
...
static rsRetVal createListner(es_str_t *port, struct cnfparamvals *pvals) {
    instanceConf_t *inst;
    int i;
    ...
    for (i = 0; i < inppblk.nParams; ++i) {
        ...
        if (!strcmp(inppblk.descr[i].name, "port")) {
            ...
            LogError(0, RS_RET_DEPRECATED,
                     "imudp: deprecated parameter inputname "
...
BEGINactivateCnfPrePrivDrop
    ...
    if (lcnfRoot == NULL) {
        LogError(0, NO_ERRCODE,
                 "imudp: no listeners could be started, "
                 "input not activated.\n");
...
static void *wrkr(void *myself) {
    ...
    statsobj.AddCounter(pWrkr->stats, UCHAR_CONSTANT("called.recvmmsg"), ctrType_IntCtr, CTR_FLAG_RESETTABLE,
                        &(pWrkr->ctrCall_recvmmsg));
    STATSCOUNTER_INIT(pWrkr->ctrCall_recvmsg, pWrkr->mutCtrCall_recvmsg);
    statsobj.AddCounter(pWrkr->stats, UCHAR_CONSTANT("called.recvmsg"), ctrType_IntCtr, CTR_FLAG_RESETTABLE,
                        &(pWrkr->ctrCall_recvmsg));
    STATSCOUNTER_INIT(pWrkr->ctrMsgsRcvd, pWrkr->mutCtrMsgsRcvd);
    statsobj.AddCounter(pWrkr->stats, UCHAR_CONSTANT("msgs.received"), ctrType_IntCtr, CTR_FLAG_RESETTABLE,
                        &(pWrkr->ctrMsgsRcvd));
...
BEGINmodInit()
    ...
    *ipIFVersProvided = CURR_MOD_IF_VERSION; /* we only support the current interface specification */
    ...
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"inputudpserverbindruleset", 0, eCmdHdlrGetWord, NULL, &cs.pszBindRuleset,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"udpserverrun", 0, eCmdHdlrGetWord, addInstance, NULL, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"udpserveraddress", 0, eCmdHdlrGetWord, NULL, &cs.pszBindAddr,
                               STD_LOADABLE_MODULE_ID));
    /* module-global config params - will be disabled in configs that are loaded
     * via module(...).
     */
    CHKiRet(regCfSysLineHdlr2((uchar *)"imudpschedulingpolicy", 0, eCmdHdlrGetWord, NULL, &cs.pszSchedPolicy,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"imudpschedulingpriority", 0, eCmdHdlrInt, NULL, &cs.iSchedPrio,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"udpservertimerequery", 0, eCmdHdlrInt, NULL, &cs.iTimeRequery,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));

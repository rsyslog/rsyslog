...
DEFobjCurrIf(tcpsrv) DEFobjCurrIf(tcps_sess) DEFobjCurrIf(net) DEFobjCurrIf(netstrm) DEFobjCurrIf(datetime)
    DEFobjCurrIf(prop) DEFobjCurrIf(statsobj)
...
static int isPermittedHost(struct sockaddr __attribute__((unused)) * addr,
                           char __attribute__((unused)) * fromHostFQDN,
...
static rsRetVal doRcvData(
    tcps_sess_t *pSess, char *buf, size_t lenBuf, ssize_t *piLenRcvd, int *oserr, unsigned *nextIODirection) {
    assert(pSess != NULL);
...
#define NO_MODIFY 0
static void getFirstWord(uchar **ppszSrc, uchar *pszBuf, size_t lenBuf, int options) {
    uchar c;
    uchar *pszSrc = *ppszSrc;

    while (*pszSrc && *pszSrc == ' ') ++pszSrc; /* skip to first non-space */

    while (*pszSrc && *pszSrc != ' ' && lenBuf > 1) {
        c = *pszSrc++;
        if (options & TO_LOWERCASE) c = tolower(c);
        *pszBuf++ = c;
        lenBuf--;
    }
 ...
 * rgerhards, 2009-05-27
 */
static rsRetVal __attribute__((format(printf, 2, 3))) sendResponse(tcps_sess_t *pSess,
                                                                   const char *const __restrict__ fmt,
                                                                   ...) {
    va_list ap;
    ssize_t len;
    uchar buf[1024];
 ...
 */
static rsRetVal doInjectMsg(uchar *szMsg, ratelimit_t *ratelimiter) {
    smsg_t *pMsg;
...
static rsRetVal injectMsg(uchar *pszCmd, tcps_sess_t *pSess) {
    ...
    CHKiRet(ratelimitNewWithThreadMode(&ratelimit, "imdiag", "injectmsg", 1));
...
static rsRetVal awaitLookupTableReload(tcps_sess_t *pSess) {
    DEFiRet;
...
static rsRetVal enableDebug(tcps_sess_t *pSess) {
    DEFiRet;
...
static rsRetVal awaitStatsReport(uchar *pszCmd, tcps_sess_t *pSess) {
    uchar subCmd[1024];
    int blockAgain = 0;
    DEFiRet;
 ...
 */
static rsRetVal ATTR_NONNULL() OnMsgReceived(tcps_sess_t *const pSess, uchar *const pRcv, const int iLenMsg) {
    uchar *pszMsg;
    uchar *pToFree = NULL;
    uchar cmdBuf[1024];
    DEFiRet;
...
static rsRetVal addTCPListener(void __attribute__((unused)) * pVal, uchar *pNewVal) {
    ...
    if (pOurTcpsrv != NULL) {
        LogError(0, NO_ERRCODE,
                 "imdiag: only a single listener is supported, "
    ...
    CHKiRet(
        tcpsrv.SetInputName(pOurTcpsrv, cnf_params, pszInputName == NULL ? UCHAR_CONSTANT("imdiag") : pszInputName));
...
static void *timeoutGuard(ATTR_UNUSED void *arg) {
    ...
    fprintf(stderr,
            "timeoutGuard: rsyslog still active after expiry of guard "
...
static rsRetVal setAbortTimeout(void __attribute__((unused)) * pVal, int timeout) {
    DEFiRet;

    if (abortTimeout != -1) {
        LogError(0, NO_ERRCODE,
                 "imdiag: abort timeout already set -"
                 "ignoring 2nd+ request");
        ABORT_FINALIZE(RS_RET_ERR);
    }
    if (timeout <= 0) {
        LogError(0, NO_ERRCODE,
                 "imdiag: $IMDiagAbortTimeout must be greater "
                 "than 0 - ignored");
        ABORT_FINALIZE(RS_RET_ERR);
    }
...
BEGINwillRun
    ...
    if (pOurTcpsrv == NULL) ABORT_FINALIZE(RS_RET_NO_RUN);
...
BEGINafterRun
    ...
    if (pRcvDummy != NULL) prop.Destruct(&pRcvDummy);
    if (pRcvIPDummy != NULL) prop.Destruct(&pRcvIPDummy);
ENDafterRun
...
BEGINmodInit()
    ...
    *ipIFVersProvided = CURR_MOD_IF_VERSION; /* we only support the current interface specification */
    ...
    if (ci_max_empty_checks != NULL) {
        ...
        if (n > 200) {
            LogError(0, RS_RET_PARAM_ERROR,
                     "env var CI_SHUTDOWN_QUEUE_EMPTY_CHECKS has "
        ...
        fprintf(stderr,
                "rsyslogd: info: imdiag does %d empty checks due to "
                "CI_SHUTDOWN_QUEUE_EMPTY_CHECKS\n",
                max_empty_checks);
    }

    /* register config file handlers */
    CHKiRet(omsdRegCFSLineHdlr(UCHAR_CONSTANT("imdiagaborttimeout"), 0, eCmdHdlrInt, setAbortTimeout, NULL,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr(UCHAR_CONSTANT("imdiagserverrun"), 0, eCmdHdlrGetWord, addTCPListener, NULL,
                               STD_LOADABLE_MODULE_ID));
    ...
    CHKiRet(omsdRegCFSLineHdlr(UCHAR_CONSTANT("imdiagserverstreamdriverauthmode"), 0, eCmdHdlrGetWord, NULL,
                               &pszStrmDrvrAuthMode, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr(UCHAR_CONSTANT("imdiagserverstreamdriverpermittedpeer"), 0, eCmdHdlrGetWord,
                               setPermittedPeer, NULL, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr(UCHAR_CONSTANT("imdiagserverinputname"), 0, eCmdHdlrGetWord, NULL, &pszInputName,
                               STD_LOADABLE_MODULE_ID));
    ...
    CHKiRet(statsobj.AddCounter(diagStats, UCHAR_CONSTANT("potentialTotalArtificialDelayInMs"), ctrType_IntCtr,
                                CTR_FLAG_NONE, &potentialArtificialDelayMs));
    STATSCOUNTER_INIT(actualArtificialDelayMs, mutActualArtificialDelayMs);
    CHKiRet(statsobj.AddCounter(diagStats, UCHAR_CONSTANT("actualTotalArtificialDelayInMs"), ctrType_IntCtr,
                                CTR_FLAG_NONE, &actualArtificialDelayMs));
    STATSCOUNTER_INIT(delayInvocationCount, mutDelayInvocationCount);
    CHKiRet(statsobj.AddCounter(diagStats, UCHAR_CONSTANT("delayInvocationCount"), ctrType_IntCtr, CTR_FLAG_NONE,

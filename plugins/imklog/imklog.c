...
DEFobjCurrIf(datetime) DEFobjCurrIf(ruleset) DEFobjCurrIf(glbl) DEFobjCurrIf(prop) DEFobjCurrIf(net)
...
static inline rsRetVal std_mod_checkRuleset(modConfData_t *const modConf) {
    ruleset_t *pRuleset;
    rsRetVal localRet;
    DEFiRet;
 ...
 */
static rsRetVal enqMsg(uchar *const __restrict__ msg,
                       uchar *pszTag,
 ...
 */
static rsRetVal parsePRI(uchar **ppSz, syslog_pri_t *piPri) {
    DEFiRet;
    syslog_pri_t i;
    uchar *pSz;
    ...
    if (*pSz != '<' || !isdigit(*(pSz + 1))) ABORT_FINALIZE(RS_RET_INVALID_PRI);
    ...
    if (*pSz != '>' || i > LOG_MAXPRI) ABORT_FINALIZE(RS_RET_INVALID_PRI);
...
rsRetVal Syslog(modConfData_t *pModConf, syslog_pri_t priority, uchar *pMsg, struct timeval *tp) {
    ...
    if (!bPRISet) {
        ...
        if (localRet != RS_RET_INVALID_PRI && localRet != RS_RET_OK) FINALIZE;
    }
    /* if we don't get the pri, we use whatever we were supplied */

    /* ignore non-kernel messages if not permitted */
    if (pModConf->bPermitNonKernel == 0 && pri2fac(priority) != LOG_KERN) FINALIZE; /* silently ignore */
 ...
 * not obtain it themselves, because they are no modules and can not query the object handler.
...
BEGINendCnfLoad
    ...
    if (!loadModConf->configSetViaV2Method) {
        ...
        if ((cs.pszPath == NULL) || (cs.pszPath[0] == '\0')) {
            ...
            if (cs.pszPath != NULL) free(cs.pszPath);
        } else {
...
BEGINmodInit()
    ...
    *ipIFVersProvided = CURR_MOD_IF_VERSION; /* we only support the current interface specification */
    ...
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"klogLocalipif", 0, eCmdHdlrGoneAway, NULL, NULL, STD_LOADABLE_MODULE_ID));
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"debugprintkernelsymbols", 0, eCmdHdlrGoneAway, NULL, NULL,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(regCfSysLineHdlr2((uchar *)"klogpath", 0, eCmdHdlrGetWord, NULL, &cs.pszPath, STD_LOADABLE_MODULE_ID,
    ...
    CHKiRet(omsdRegCFSLineHdlr((uchar *)"klogusesyscallinterface", 0, eCmdHdlrGoneAway, NULL, NULL,
                               STD_LOADABLE_MODULE_ID));
    CHKiRet(regCfSysLineHdlr2((uchar *)"klogpermitnonkernelfacility", 0, eCmdHdlrBinary, NULL, &cs.bPermitNonKernel,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"klogconsoleloglevel", 0, eCmdHdlrInt, NULL, &cs.console_log_level,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"kloginternalmsgfacility", 0, eCmdHdlrFacility, NULL, &cs.iFacilIntMsg,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"klogparsekerneltimestamp", 0, eCmdHdlrBinary, NULL, &cs.bParseKernelStamp,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
    CHKiRet(regCfSysLineHdlr2((uchar *)"klogkeepkerneltimestamp", 0, eCmdHdlrBinary, NULL, &cs.bKeepKernelStamp,
                              STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));

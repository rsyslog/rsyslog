...
typedef struct io_req_s {
	STAILQ_ENTRY(io_req_s)
	link;
	epolld_t *epd;
} io_req_t;

typedef struct io_q_s {
	STAILQ_HEAD(ioq_s, io_req_s)
	q;
    ...
    unsigned *const __restrict__ pnMsgs)
{
	...
	if (pThis->inputState == eInOctetCnt) {
		...
		if ((c == '\n') || ((pThis->pLstn->pSrv->iAddtlFrameDelim != TCPSRV_NO_ADDTL_DELIMITER) && (c == pThis->pLstn->pSrv->iAddtlFrameDelim))) {
		...
		if (pThis->eFraming == TCP_FRAMING_OCTET_STUFFING) {
			if (pThis->iMsg >= iMaxLine) {
				...
				int i = 1;
				char currBuffChar;
				while (i < buffLen && ((currBuffChar = (*buff)[i]) != '\n' && (pThis->pLstn->pSrv->iAddtlFrameDelim == TCPSRV_NO_ADDTL_DELIMITER || currBuffChar != pThis->pLstn->pSrv->iAddtlFrameDelim))) {
					i++;
				}
			...
			if ((c == '\n') || ((pThis->pLstn->pSrv->iAddtlFrameDelim != TCPSRV_NO_ADDTL_DELIMITER) && (c == pThis->pLstn->pSrv->iAddtlFrameDelim))) { /* record delimiter? */
				doSubmitMsg(pThis, stTime, ttGenTime, pMultiSub);
				++(*pnMsgs);
				pThis->inputState = eAtStrtFram;
			} else {

...
static void ATTR_NONNULL()
		doFunct_Base64Dec(struct cnffunc *__restrict__ const func,
						  struct svar *__restrict__ const ret,
						  void *__restrict__ const usrptr,
						  wti_t *__restrict__ const pWti)
{
	...
	if( len == 0 ) {
		if( strlen(cstr) != 0 ) {
			DBGPRINTF("base64_enc: Failed to determine decoded payload length of '%s' \n", cstr);
		}
		goto finalize_it;
	}
	...
	DBGPRINTF("rainerscript: (base64_enc) Decoded data = '%.*s' \n", (int)len, res);
	...
	if(cmpop == CMP_EQ) {
		...
		for(i = 0 ; (r == 0) && (i < ar->nmemb) ; ++i) {
			...
			case CMP_ENDSWITH: {
					...
					r = len_l >= len_suffix &&
					    (memcmp(es_getBufAddr(estr_l) + len_l - len_suffix,
	...
	wti_t *__restrict__ const pWti)
{
	...
	case CMP_ENDSWITH:
		...
		if(expr->r->nodetype == 'A') {
			...
			ret->d.n = estr_l->lenStr >= estr_r->lenStr &&
				(memcmp(es_getBufAddr(estr_l) + estr_l->lenStr - estr_r->lenStr,

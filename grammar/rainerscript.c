    ...
    struct cnfparamvals *val)
{
	...
	if (es_strlen(valnode->val.d.estr) == 4) {
		c = es_getBufAddr(valnode->val.d.estr);
		if ((c[0] == '0') && (c[1] >= '0' && c[1] <= '7') && (c[2] >= '0' && c[2] <= '7') && (c[3] >= '0' && c[3] <= '7')) {
			fmtOK = 1;
		}
	}
    ...
    struct cnfparamvals *val)
{
	...
	case 'A':
		...
		break;
	default:
		parser_errmsg("parameter '%s' must be an array, but is a "
			      "different datatype",
		    param->name);
		r = 0;
		break;
    ...
    struct svar *__restrict__ const replaceWithVal)
{
	...
	if (freeOperand)
		es_deleteStr(str);
	if (freeFind)
		es_deleteStr(findStr);
	if (freeReplacement)
		es_deleteStr(replaceWithStr);
	return res;
}
    ...
    struct svar *__restrict__ const ret)
{
	...
	if (var->prop.id == PROP_CEE ||
		...
		if (json != NULL) {
			...
			ret->datatype = 'S';
			ret->d.estr = (localRet == RS_RET_OK) ? es_newStrFromCStr((char *)cstr, strlen((char *)cstr))
							      : es_newStr(1);
 ...
 */
void cnfexprEval(const struct cnfexpr *__restrict__ const expr, struct svar *__restrict__ const ret,
    void *__restrict__ const usrptr)
{
 ...
 * important.
 */
int cnfexprEvalBool(struct cnfexpr *__restrict__ const expr, void *__restrict__ const usrptr)
{
	int convok;
	struct svar ret;
...
cnfstmtNewSet(char *var, struct cnfexpr *expr, int force_reset)
{
	...
	if ((cnfstmt = cnfstmtNew(S_SET)) != NULL) {
		if (propNameToID((uchar *)var, &propid) == RS_RET_OK && (propid == PROP_CEE || propid == PROP_LOCAL_VAR || propid == PROP_GLOBAL_VAR)) {

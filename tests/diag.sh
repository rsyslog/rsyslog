...
wait_startup() {
	...
	if [ -f "$RSYSLOG_PIDBASE$1.pid" ]; then
		...
		echo "ABORT! rsyslog pid file $RSYSLOG_PIDBASE$1.pid does not exist!"
		error_exit 1 stacktrace
	fi
	if [ $? -ne 0 ]; then
		echo "ABORT! rsyslog pid crashed immediately during startup!"
		error_exit 1 stacktrace
	fi
...
error_exit() {
	...
	for corefile in core* /cores/core-* /cores/core.* $(find . -name "core.*" -o -name "*.core" 2>/dev/null); do
		if [ -f "$corefile" ]; then
			...
			echo "Core file size: $(ls -lh "$corefile" | awk '{print $5}')"
	...
	if [ $core_dumps_found -eq 0 ]; then
		...
		if [ "$(uname)" == "Darwin" ]; then
			...
			if [ -d "~/Library/Logs/CrashReporter" ]; then
				echo "    User crash logs: $(find ~/Library/Logs/CrashReporter -name '*rsyslogd*' -mtime -1 2>/dev/null | wc -l) found"
			fi
		else

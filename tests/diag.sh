...
# IMDIAG_INJECTMSG_DELAY_MS
#               Milliseconds to wait between numeric messages generated by
#               injectmsg/injectmsg2. Default is 0, which preserves the
#               historical burst injection behavior.
#
#
# EXIT STATES
# 0 - ok
...
## Set IMDIAG_INJECTMSG_DELAY_MS to add a delay in milliseconds between
## generated numeric messages. The default is 0/no delay.
injectmsg() {
	if [ "$3" != "" ] ; then
		printf 'warning: injectmsg shell helper ignores extra args; use IMDIAG_INJECTMSG_DELAY_MS for delay, extra arg is %s\n' "$3"
	fi
	msgs=${2:-${NUMMESSAGES:-1}}
	delay_ms=${IMDIAG_INJECTMSG_DELAY_MS:-0}
	echo injecting $msgs messages
	if [ "$delay_ms" = "0" ]; then
		echo injectmsg "${1:-0}" "$msgs" | $TESTTOOL_DIR/diagtalker -p$IMDIAG_PORT || error_exit  $?
	else
		echo injectmsg "${1:-0}" "$msgs" "$delay_ms" | $TESTTOOL_DIR/diagtalker -p$IMDIAG_PORT || error_exit  $?
	fi
}
...
injectmsg2() {
	...
	if [ "$3" != "" ] || [ "$4" != "" ] ; then
		printf 'warning: injectmsg2 ignores deprecated extra arguments: %s %s\n' "$3" "$4"
	fi
	if [ "$delay_ms" = "0" ]; then
		echo injectmsg "${1:-0}" "$msgs" | $TESTTOOL_DIR/diagtalker -p$IMDIAG_PORT2 || error_exit  $?
	else
		echo injectmsg "${1:-0}" "$msgs" "$delay_ms" | $TESTTOOL_DIR/diagtalker -p$IMDIAG_PORT2 || error_exit  $?
	fi

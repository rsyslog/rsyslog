...
# $2 - ES port
es_getdata() {
	curl -XPUT "http://localhost:${2:-$ES_PORT}/rsyslog_testbench/_settings" -d '{ "index" : { "max_result_window" : '${1:-$NUMMESSAGES}' } }'
	curl --silent localhost:${2:-$ES_PORT}/rsyslog_testbench/_search?size=${1:-$NUMMESSAGES} > $RSYSLOG_DYNNAME.work
	head $RSYSLOG_DYNNAME.work
	tail $RSYSLOG_DYNNAME.work

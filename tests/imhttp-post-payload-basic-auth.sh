...
input(type="imhttp" endpoint="/postrequest"
      ruleset="ruleset"
      addmetadata="on"
      basicauthfile="testsuites/htpasswd")
...
ret=$(curl -s -H Content-Type:application/json -H "BAZ: skat" -H "daddle: doodle" -H "fiddle: faddle" \
  --user user1:badpass --write-out '%{http_code}' \
  http://localhost:$IMHTTP_PORT/postrequest -d '[{"foo":"bar","bar":"foo"},{"one":"two","three":"four"}]')

echo "$ret" | grep "Error 401: Unauthorized"
if [[ $? != 0 ]];
then

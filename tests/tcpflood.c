...
static void initRELP_PLAIN(void) {
    CHKRELP(relpEngineConstruct(&pRelpEngine));
    CHKRELP(relpEngineSetDbgprint(pRelpEngine, verbose ? relp_dbgprintf : NULL));
    CHKRELP(relpEngineSetEnableCmd(pRelpEngine, (unsigned char *)"syslog", eRelpCmdState_Required));
...
int openConn(const int connIdx) {
    ...
    if (transport == TP_RELP_PLAIN || transport == TP_RELP_TLS) {
        ...
        if (transport == TP_RELP_TLS) {
            ...
            if (relpPermittedPeer != NULL && relpCltAddPermittedPeer(relpClt, relpPermittedPeer) != RELP_RET_OK) {
                fprintf(stderr, "could not set Permitted Peer: %s\n", relpPermittedPeer);
                exit(1);
            }
    #if defined(HAVE_RELPENGINESETTLSCFGCMD)
            /* Check for Custom Config string */
            if (customConfig != NULL && relpCltSetTlsConfigCmd(relpClt, customConfig) != RELP_RET_OK) {
...
static int sendPlainTCP(const int socknum, const char *const buf, const size_t lenBuf, int *const ret_errno) {
    size_t lenSent;
    int r;
...
int sendMessages(struct instdata *inst) {
    ...
    if (bShowProgress) printf("\r%8.8d %s sent", 0, statusText);
    while (i < inst->numMsgs) {
        ...
        if (lenBuf == 0) break; /* terminate when no message could be generated */
        if (transport == TP_TCP) {
            ...
            relp_ret = relpCltSendSyslog(relpCltArray[socknum], (unsigned char *)buf, lenBuf);
        ...
        if (lenSend != lenBuf) {
            ...
            if (lenSend == 0) {
                ...
                printf(
                    "tcpflood: send() failed \"%s\" at socket %d, index %u, "
                    "msgNum %lld, lenSend %zd, lenBuf %zd\n",
                    errStr, sockArray[socknum], i, inst->numSent, lenSend, lenBuf);
            }
            fflush(stderr);
        ...
        if (i % show_progress_interval == 0) {
            if (bShowProgress) printf("\r%8.8u", i);
        }
            ...
            if (rand() > (int)(RAND_MAX * dbRandConnDrop)) {
                if (transport == TP_TLS && offsSendBuf != 0) {
                    ...
                    if (lenSend != offsSendBuf) {
                        fprintf(stderr,
                                "tcpflood: error in send function for conn %d "
    ...
    if (!bSilent) printf("\r%8.8u %s sent\n", i, statusText);
 ...
 */
static void endTiming(struct timeval *tvStart, struct runstats *stats) {
    long sec, usec;
    ...
    if (runtime < stats->minRuntime) stats->minRuntime = runtime;
    if (runtime > stats->maxRuntime) stats->maxRuntime = runtime;
 ...
 */
static void genStats(struct runstats *stats) {
    long unsigned avg;
    avg = stats->totalRuntime / stats->numRuns;

    if (bCSVoutput) {
        printf("#numRuns,TotalRuntime,AvgRuntime,MinRuntime,MaxRuntime\n");
        printf("%d,%llu.%3.3d,%lu.%3.3lu,%lu.%3.3lu,%lu.%3.3lu\n", stats->numRuns, stats->totalRuntime / 1000,
               (int)stats->totalRuntime % 1000, avg / 1000, avg % 1000, stats->minRuntime / 1000,
               stats->minRuntime % 1000, stats->maxRuntime / 1000, stats->maxRuntime % 1000);
    } else {
        printf("Runs:     %d\n", stats->numRuns);
        printf("Runtime:\n");
...
static int runTests(void) {
    ...
    run = 1;
    while (1) { /* loop broken inside */
        if (!bSilent) printf("starting run %d\n", run);
        prepareGenerators();
        ...
        if (strlen(pCurrentPos) > 0) {
            ...
            if (pNextPos != NULL) {
                ...
                if (iConfErr > 0) {
                    ...
                    printf(
                        "tcpflood: error, adding Command: %s:%s "
...
static void initTLS(void) {
    ...
    if (r != GNUTLS_E_SUCCESS) {
        ...
        if (r == GNUTLS_E_SUCCESS) {
            printf(
                "Tried swapping files, this seems to work "
...
static int sendTLS(int __attribute__((unused)) i,
                   char __attribute__((unused)) * buf,
                   size_t __attribute__((unused)) lenBuf) {
    return 0;
}
static void closeTLSSess(int __attribute__((unused)) i) {}
...
int main(int argc, char *argv[]) {
...
#else
                    fprintf(stderr,
                            "compiled without gnutls/openssl TLS support: "
                            "\"-Ttls\" not supported!\n");
                    exit(1);
#endif
...
#else
                    fprintf(stderr,
                            "compiled without openssl TLS support: "
                            "\"-Tdtls\" not supported!\n");
                    exit(1);
#endif
                } else {
    ...
    if (numConnections >= (os_max_fds - 20)) {
        fprintf(stderr,
                "We are asked to use %d connections, but the OS permits only %d "
                "open file descriptors.\n",
                numConnections, os_max_fds);
        if (softLimitConnections) {
            numConnections = os_max_fds - 20;
            fprintf(stderr,
                    "We reduced the actual number of connections to %d. "
                    "This leaves some room for opening files.\n",
                    numConnections);
        } else {
            fprintf(stderr,
                    "Connection count is hard requirement, so we "
                    "error-terminate\n");
            exit(1);
        }
    }
    ...
    if (bStatsRecords && waittime) {
        fprintf(stderr,
                "warning: generating performance stats and using a waittime "
                "is somewhat contradictory!\n");
    }

    if (!isatty(1) || bSilent) bShowProgress = 0;
    ...
    if (tlsKeyFile != NULL || tlsCertFile != NULL) {
        if (transport != TP_TLS && transport != TP_DTLS && transport != TP_RELP_TLS) {
            printf(
                "error: TLS certificates were specified, but TLS is NOT enabled: "
    ...
    if (transport == TP_TLS) {
        if (tlsKeyFile == NULL || tlsCertFile == NULL) {
            printf(
                "error: transport TLS is specified (-Ttls), -z and -Z must also "
                "be specified\n");
            exit(1);
        }
        ...
        if (tlsKeyFile == NULL || tlsCertFile == NULL) {
            printf(
                "error: transport DTLS is specified (-Tdtls), -z and -Z must also "
                "be specified\n");
            exit(1);
        }
#if defined(ENABLE_OPENSSL)
    ...
    if (nConnDrops > 0 && !bSilent) printf("-D option initiated %ld connection closures\n", nConnDrops);

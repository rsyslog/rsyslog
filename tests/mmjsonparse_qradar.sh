...
if [ -n "$PYTHON" ] && command -v "$PYTHON" >/dev/null 2>&1; then
    ...
    if $PYTHON validate_json_equality.py $srcdir/testsuites/qradar_json $RSYSLOG_OUT_LOG "<01> @cee:"; then
    ...
    if bash validate_json_shell.sh $srcdir/testsuites/qradar_json $RSYSLOG_OUT_LOG "<01> @cee:"; then

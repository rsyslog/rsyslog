...
TARGETS_FILE="${INSTALL_DIR:-/opt/rosi-collector}/prometheus-targets/nodes.yml"
...
Commands:
  ...
  remove <ip:port>            Remove a target
...
check_file() {
  if [ ! -f "$TARGETS_FILE" ]; then
    ...
    echo "Run initcentralenv.sh first to create the central stack" >&2
    exit 1
  fi
}
...
cmd_remove() {
  if [ $# -lt 1 ]; then
    ...
    echo "Usage: $(basename "$0") remove <ip:port>" >&2
    exit 1
  fi

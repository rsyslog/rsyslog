...
## @fn configure_server_impstats()
## @brief Enable rsyslog internal performance statistics (impstats)
## @description
##   Configures the impstats module on the host rsyslog to report internal
##   metrics like queue depth, message rates, and action status. These
##   metrics are forwarded to ROSI Collector and displayed in the
##   "rsyslog Client Health" Grafana dashboard.
##
## @env SERVER_IMPSTATS_ENABLED Set to "true" to enable in non-interactive mode
## @env SERVER_IMPSTATS_INTERVAL Reporting interval in seconds (default: 60)

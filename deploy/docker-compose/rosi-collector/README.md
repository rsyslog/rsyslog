...
└─────────────────┘                      └─────────────────┘
         ...
         ▲ Server also forwards its own logs + impstats to localhost:10514
...
- **impstats** - Enable rsyslog performance metrics (queue depth, message rates)
...
sudo TRAEFIK_DOMAIN=logs.example.com \
     ...
     SERVER_IMPSTATS_ENABLED=true \
     SERVER_IMPSTATS_INTERVAL=60 \
     ./scripts/init.sh
```
...
- Configure rsyslog impstats for server performance monitoring (optional)
...
| `SERVER_IMPSTATS_ENABLED` | `false` | Enable rsyslog impstats on server |
| `SERVER_IMPSTATS_INTERVAL` | `60` | impstats reporting interval (seconds) |

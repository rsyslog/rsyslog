...
1. **`client-rsyslog-forward-minimal.conf`** (Recommended)
...
```bash
CONFIG_FILE="client-rsyslog-forward-minimal.conf"
```
...
- Consider using TLS/SSL for log forwarding in high-security environments (requires additional configuration)
- Queue files may contain sensitive log data - ensure proper file permissions

## Performance Monitoring (impstats)

Enable rsyslog internal statistics to monitor queue depths, message rates, and failures.

### Installation

The automated install script prompts to enable impstats. To enable manually:

1. Create `/etc/rsyslog.d/10-impstats.conf`:

```bash
sudo cat > /etc/rsyslog.d/10-impstats.conf << 'EOF'
...
3. View metrics in Grafana dashboard: **Rsyslog Client Health**
...
| `messages.sent` | omfwd | Messages forwarded |
| `bytes.sent` | omfwd | Bytes forwarded |
| `failed` | core.action | Action failures |
| `maxrss` | impstats | Memory usage (KB) |
| `openfiles` | impstats | Open file descriptors |
...
```bash
sudo rm /etc/rsyslog.d/10-impstats.conf
sudo systemctl restart rsyslog
```

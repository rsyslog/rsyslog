...
static char *ocsp_make_cache_key(X509 *cert, X509 *issuer) {
    ...
    /* Format: serial-number:issuer-hash */
    BIGNUM *bn = ASN1_INTEGER_to_BN(serial, NULL);
    if (!bn) return NULL;

    char *serial_hex = BN_bn2hex(bn);
    BN_free(bn);
    if (!serial_hex) return NULL;

    key_len = strlen(serial_hex) + (md_len * 2) + 2;
    key = malloc(key_len);
    if (key) {
        snprintf(key, key_len, "%s:", serial_hex);
        for (unsigned int i = 0; i < md_len; i++) {
            snprintf(key + strlen(key), 3, "%02x", md[i]);
        }
    }
...
static void ocsp_cache_store(const char *cache_key, int cert_status, time_t ttl) {
    ...
    /* Add new entry */
    entry = malloc(sizeof(ocsp_cache_entry_t));
    if (entry) {
        entry->cache_key = strdup(cache_key);
        if (!entry->cache_key) {
            /* strdup failed, free entry and return */
            free(entry);
            pthread_mutex_unlock(&ocsp_cache_mutex);
            LogError(0, RS_RET_OUT_OF_MEMORY, "OCSP: Failed to allocate cache key\n");
            return;
        }
        entry->cert_status = cert_status;
        entry->expires_at = time(NULL) + ttl;
        entry->next = ocsp_cache_head;
        ocsp_cache_head = entry;
        ocsp_cache_count++;
...
static int ocsp_check_validate_response_and_cert(OCSP_RESPONSE *rsp,
    ...
    if (cache_key) {
        ...
        if (nextupd) {
            ...
            if (ASN1_TIME_to_tm(nextupd, &tm) != 0) {
                time_t next_update_time = mktime(&tm);
                if (next_update_time != (time_t)-1) {
                    time_t now = time(NULL);
                    if (next_update_time > now) {
                        cache_ttl = next_update_time - now;
                    }
                } else {
                    dbgprintf("OCSP: mktime() failed, using default TTL\n");
                }
            } else {
                dbgprintf("OCSP: ASN1_TIME_to_tm() failed, using default TTL\n");
            }
        }

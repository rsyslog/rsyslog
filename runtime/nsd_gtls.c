...
static rsRetVal gtlsChkPeerName(nsd_gtls_t *pThis, gnutls_x509_crt_t *pCert) {
    ...
    while (!bFoundPositiveMatch) { /* loop broken below */
        ...
        if (gnuRet < 0)
            ...
            inet_ntop(((szAltNameLen == sizeof(struct in6_addr)) ? AF_INET6 : AF_INET), &szAltName, ipAddress, INET6_ADDRSTRLEN);
            dbgprintf("subject alt ipAddr: '%s'\n", ipAddress);
            snprintf((char*)lnBuf, sizeof(lnBuf), "IPaddress: %s; ", ipAddress);
            CHKiRet(rsCStrAppendStr(pStr, lnBuf));
            CHKiRet(gtlsChkOnePeerName(pThis, ipAddress, &bFoundPositiveMatch));
            /* do NOT break, because there may be multiple ipAddr's! */
        }
        ++iAltName;
    }

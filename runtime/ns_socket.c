...
ns_switch(const uchar *ns)
{
	...
	ns_fd = open(nsPath, 0);
	if (ns_fd < 0) {
		LogError(0, RS_RET_IO_ERROR, "%s: could not change to namespace '%s': %s",
					__func__, ns, strerror(errno));
...
finalize_it:
	...
	if ((ns_fd >= 0) && (close(ns_fd) != 0)) {
		LogError(0, RS_RET_IO_ERROR, "%s: failed to close namespace '%s': %s",
					__func__, ns, strerror(errno));
	}
	RETiRet;
}
...
ns_pop(int *fd)
{
	...
	if (*fd >= 0) {
		if (setns(*fd, CLONE_NEWNET) != 0) {
			LogError(0, RS_RET_IO_ERROR, "%s: could not return to startup namespace: %s",
...
finalize_it:
	if (*fd >= 0 && close(*fd) != 0) {
		LogError(0, RS_RET_IO_ERROR, "%s: could not close startup namespace fd: %s",
					__func__, strerror(errno));
		*fd = -1;
	}
	RETiRet;
...
ns_push(int *fd)
{
	 ...
	 */
	if (*fd >= 0 && close(*fd) != 0)
		LogError(0, RS_RET_IO_ERROR, "%s: could not close previous namespace: %s",
				__func__, strerror(errno));
	*fd = open("/proc/self/ns/net", O_RDONLY);
	if (*fd == -1) {
		LogError(0, RS_RET_IO_ERROR, "%s: could not access startup namespace: %s",
				__func__, strerror(errno));
		ABORT_FINALIZE(RS_RET_IO_ERROR);
	}
...
rsRetVal
ns_socket(int *fdp, int domain, int type, int protocol, const uchar *ns)
{
	DEFiRet;
	int fd = -1;
	...
	if (ns && *ns) {
		CHKiRet(ns_push(&ns_fd));
		CHKiRet(ns_switch(ns));
	}
#endif /* def HAVE_SETNS */
	*fdp = fd = socket(domain, type, protocol);
	if(fd == -1) {
		LogError(0, RS_RET_NO_SOCKET, "%s: socket(%d, %d, %d) failed: %s",
				__func__, domain, type, protocol, strerror(errno));
		ABORT_FINALIZE(RS_RET_NO_SOCKET);
	}
finalize_it:
#ifdef HAVE_SETNS
	iRet_pop = ns_pop(&ns_fd);
	if(iRet_pop != RS_RET_OK && iRet == RS_RET_OK)
		iRet = iRet_pop;
#endif /* def HAVE_SETNS */

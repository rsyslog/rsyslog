 ...
 * @returns 0 if ok, something else otherwise
 */
int rsgt_tlvrdHeader(FILE *fp, uchar *hdr)
{
	int r;
...
rsgt_tlvrdRecHash(FILE *fp, FILE *outfp, imprint_t **imp)
{
	...
	if (outfp != NULL)
		if ((r = rsgt_tlvwrite(outfp, &rec)) != 0)
			goto done;
	r = 0;
done:
	if (r == 0 && rsgt_read_debug)
...
void rsgt_objfree(uint16_t tlvtype, void *obj)
{
	...
	case 0x0904:
		...
		break;
	default:
		fprintf(stderr, "rsgt_objfree: unknown tlv record %4.4x\n",
		    tlvtype);
		break;
	}
	free(obj);
 ...
 * @returns 0 if ok, something else otherwise
 */
int rsgt_getBlockParams(FILE *fp, uint8_t bRewind, block_sig_t **bs, block_hdr_t **bh,
...
int rsgt_vrfy_nextRec(gtfile gf, FILE *sigfp, FILE *nsigfp,
...
int verifyBLOCK_HDR(FILE *sigfp, FILE *nsigfp)
{
	...
	tlvrecord_t rec;
	block_hdr_t *bh = NULL;
	if ((r = rsgt_tlvrd(sigfp, &rec, &bh)) != 0)
		goto done;
	...
	if (nsigfp != NULL)
		if ((r = rsgt_tlvwrite(nsigfp, &rec)) != 0)
			goto done;
done:
	/*	if (r == 0 || r == RSGTE_IO)*/ {
 ...
 */
int verifyBLOCK_SIG(block_sig_t *bs, gtfile gf, FILE *sigfp, FILE *nsigfp,
    uint8_t bExtend, gterrctx_t *ectx)
{
	...
	if (!(gtstate == GT_OK && vrfyInf->verification_errors == GT_NO_FAILURES)) {
	...
	if (bExtend)
		if ((r = rsgt_extendSig(timestamp, &rec, ectx)) != 0)
			goto done;
	...
	while (1) { /* we will err out on EOF */
		...
		if (rRead == 0 /*|| rRead == RSGTE_EOF*/) {
			...
			case 0x0901:
				...
				if ((r = rsgt_tlvwrite(newsigfp, &rec)) != 0)
					goto done;

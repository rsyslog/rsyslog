...
static void addPadding(osslfile pF, uchar* buf, size_t* plen) {
    unsigned i;
    size_t nPad;
...
static rsRetVal eiWriteRec(osslfile gf, const char* recHdr, size_t lenRecHdr, const char* buf, size_t lenBuf) {
    struct iovec iov[3];
...
static rsRetVal eiOpenRead(osslfile gf) {
    DEFiRet;
...
static rsRetVal eiRead(osslfile gf) {
    ssize_t nRead;
    DEFiRet;
...
static int eiReadChar(osslfile gf) {
    int c;
...
static rsRetVal eiCheckFiletype(osslfile gf) {
    char hdrBuf[128];
 ...
 */
static rsRetVal eiGetRecord(osslfile gf, char* rectype, char* value) {
    unsigned short i, j;
    int c;
    DEFiRet;
...
static rsRetVal eiGetIV(osslfile gf, uchar* iv, size_t leniv) {
...
static rsRetVal eiGetEND(osslfile gf, off64_t* offs) {
    char rectype[EIF_MAX_RECTYPE_LEN + 1];
    char value[EIF_MAX_VALUE_LEN + 1];
    DEFiRet;
...
static rsRetVal eiOpenAppend(osslfile gf) {
    rsRetVal localRet;
    DEFiRet;
...
static rsRetVal __attribute__((nonnull(2))) eiWriteIV(osslfile gf, const uchar* const iv) {
 ...
 */
static void eiClose(osslfile gf, off64_t offsLogfile) {
    char offs[21];
    size_t len;
    if (gf->fd == -1) return;
    if (gf->openMode == 'w') {
...
static rsRetVal osslfileConstruct(osslctx ctx, osslfile* pgf, uchar* logfn) {
    char fn[MAXFNAME + 1];
    osslfile gf;
    DEFiRet;

...
/* static data */
DEFobjStaticHelpers;
DEFobjCurrIf(zlibw) DEFobjCurrIf(zstdw)
 ...
 */
static const char *getFileDebugName(const strm_t *const pThis) {
    return (pThis->pszCurrFName == NULL) ? ((pThis->pszFName == NULL) ? "N/A" : (char *)pThis->pszFName)
    ...
    if (iRet != RS_RET_OK) {
        if (iRet == RS_RET_SIZELIMITCMD_DIDNT_RESOLVE) {
            LogError(0, RS_RET_ERR,
                     "file size limit cmd for file '%s' "
 ...
 */
static rsRetVal doSizeLimitProcessing(strm_t *pThis) {
...
static rsRetVal doPhysOpen(strm_t *pThis) {
    ...
    if (pThis->cryprov != NULL) {
        CHKiRet(pThis->cryprov->OnFileOpen(pThis->cryprovData, pThis->pszCurrFName, &pThis->cryprovFileData,
...
static rsRetVal strmSetCurrFName(strm_t *pThis) {
    DEFiRet;
    ...
    if (pThis->sType == STREAMTYPE_FILE_CIRCULAR) {
        ...
        if (pThis->pszDir == NULL) {
            if ((pThis->pszCurrFName = ustrdup(pThis->pszFName)) == NULL) ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
        } else {
            CHKiRet(genFileName(&pThis->pszCurrFName, pThis->pszDir, pThis->lenDir, pThis->pszFName, pThis->lenFName,
                                -1, 0));
        }
 ...
 */
static rsRetVal CheckFileChange(strm_t *pThis) {
    struct stat statName;
    DEFiRet;

    CHKiRet(strmSetCurrFName(pThis));
    if (stat((char *)pThis->pszCurrFName, &statName) == -1) ABORT_FINALIZE(RS_RET_IO_ERROR);
    DBGPRINTF(
        "CheckFileChange: stream/after deserialize checking for file change "
        "on '%s', inode %u/%u, size/currOffs %llu/%llu\n",
        pThis->pszCurrFName, (unsigned)pThis->inode, (unsigned)statName.st_ino, (long long unsigned)statName.st_size,
...
static rsRetVal strmOpenFile(strm_t *pThis) {
    ...
    if (pThis->fd != -1) ABORT_FINALIZE(RS_RET_OK);

    free(pThis->pszCurrFName);
    pThis->pszCurrFName = NULL; /* used to prevent mem leak in case of error */

    if (pThis->pszFName == NULL) ABORT_FINALIZE(RS_RET_FILE_PREFIX_MISSING);
    ...
    if (pThis->tOperationsMode == STREAMMODE_WRITE_APPEND) {
        ...
        if (offset != 0) {
            LogError(0, 0,
                     "file '%s' opened for truncate write, but "
 ...
 */
static void strmWaitAsyncWriterDone(strm_t *pThis) {
 ...
 */
static rsRetVal ATTR_NONNULL() strmHandleEOF(strm_t *const pThis) {
    DEFiRet;
...
static rsRetVal ATTR_NONNULL()
    ...
    LogMsg(err_no, RS_RET_FILE_TRUNCATED, LOG_WARNING,
           "file '%s': truncation detected, "
 ...
 */
static rsRetVal ATTR_NONNULL() checkTruncation(strm_t *const pThis) {
    DEFiRet;
    off64_t ret;
    ...
    if (ret < 0) {
        iRet = rereadTruncated(pThis, errno, "cannot seek backward to begin of last block", backseek);
        FINALIZE;
    }

    const ssize_t lenRead = read(pThis->fd, pThis->pIOBuf_truncation, pThis->iBufPtrMax);
    if (lenRead != (ssize_t)pThis->iBufPtrMax) {
        iRet = rereadTruncated(pThis, errno, "last block could not be re-read", lenRead);
        FINALIZE;
    }
 ...
 * rgerhards, 2008-02-13
 */
static rsRetVal strmReadBuf(strm_t *pThis, int *padBytes) {
    DEFiRet;
    int bRun;
    long iLenRead;
            ...
            if (pThis->cryprov != NULL) {
                ...
                *padBytes = iLenRead - actualDataLen;
                ...
                DBGOPRINT((obj_t *)pThis,
                          "encrypted file %d pad bytes %d, actual "
                          "data %ld\n",
...
void strmDebugOutBuf(const strm_t *const pThis) {
    ...
    if (strtIdx < 0) strtIdx = 0;
    DBGOPRINT((obj_t *)pThis, "strmRead ungetc %d, index %zd, max %zd, buf '%.*s', CURR: '%.*s'\n", pThis->iUngetC,
              pThis->iBufPtr, pThis->iBufPtrMax, (int)pThis->iBufPtrMax - strtIdx, pThis->pIOBuf + strtIdx,
              (int)(pThis->iBufPtrMax - pThis->iBufPtr), pThis->pIOBuf + pThis->iBufPtr);
}
 ...
 */
static rsRetVal ATTR_NONNULL(1, 2) strmReadLine(strm_t *const pThis,
                                                cstr_t **ppCStr,
                                                uint8_t mode,
                                                sbool bEscapeLF,
                                                const uchar *const escapeLFString,
                    ...
                    if (pThis->bPrevWasNL && escapeLFString_len > 0) {
                        ...
                        if (bEscapeLF) {
                            if (escapeLFString == NULL) {
                                ...
                                CHKiRet(rsCStrAppendStrWithLen(*ppCStr, escapeLFString, escapeLFString_len));
                            }
                        } else {
                            CHKiRet(cstrAppendChar(*ppCStr, c));
                        }
...
int strmReadMultiLine_isTimedOut(const strm_t *const __restrict__ pThis) {
     ...
     */
    DBGPRINTF(
        "strmReadMultiline_isTimedOut: prevMsgSeg %p, readTimeout %d, "
        "lastRead %lld\n",
        pThis->prevMsgSegment, pThis->readTimeout, (long long)pThis->lastRead);
    return ((pThis->readTimeout) && (pThis->prevMsgSegment != NULL) &&
...
rsRetVal ATTR_NONNULL(1, 2) strmReadMultiLine(strm_t *pThis,
                                              ...
                                              regex_t *start_preg,
                                              regex_t *end_preg,
                                              const sbool bEscapeLF,
                                              const uchar *const escapeLFString,
                                              const sbool discardTruncatedMsg,
                                              const sbool msgDiscardingError,
                                              int64 *const strtOffs) {
    uchar c;
    uchar finished = 0;
    ...
    do {
        ...
        const int isStartMatch =
            start_preg ? !regexec(start_preg, (char *)rsCStrGetSzStrNoNULL(thisLine), 0, NULL, 0) : 0;
        const int isEndMatch = end_preg ? !regexec(end_preg, (char *)rsCStrGetSzStrNoNULL(thisLine), 0, NULL, 0) : 0;
...
finalize_it:
    *strtOffs = pThis->strtOffs;
    ...
    if (iRet == RS_RET_OK) {
        ...
        if (pThis->readTimeout && (pThis->prevMsgSegment != NULL) && (tCurr > pThis->lastRead + pThis->readTimeout)) {
...
static rsRetVal strmConstructFinalize(strm_t *pThis) {
    ...
    if (pThis->iZipLevel) { /* do we need a zip buf? */
        if (pThis->compressionDriver == STRM_COMPRESS_ZSTD) {
            ...
            if (localRet != RS_RET_OK) {
                pThis->iZipLevel = 0;
                LogError(0, localRet,
                         "stream was requested with zstd compression mode, "
            ...
            if (localRet != RS_RET_OK) {
                pThis->iZipLevel = 0;
                LogError(0, localRet,
                         "stream was requested with zip mode, but zlibw "
    ...
    if (pThis->bSync && !pThis->bIsTTY && pThis->pszDir != NULL) {
        ...
        if (pThis->fdDir == -1) {
            ...
            rs_strerror_r(err, errStr, sizeof(errStr));
            DBGPRINTF(
                "error %d opening directory file for fsync() use - fsync for directory "
                "disabled: %s\n",
                errno, errStr);
        }
    }
    ...
    if (pThis->bAsyncWrite) {
        ...
        pThis->bStopWriter = 0;
        if (pthread_create(&pThis->writerThreadID, &default_thread_attr, asyncWriterThread, pThis) != 0)
...
BEGINobjDestruct(strm) /* be sure to specify the object type also in END and CODESTART macros! */
    ...
    if (pThis->bAsyncWrite) /* Note: mutex will be unlocked in strmCloseFile/stopWriter! */
    ...
    if (pThis->prevLineSegment) cstrDestruct(&pThis->prevLineSegment);
    if (pThis->prevMsgSegment) cstrDestruct(&pThis->prevMsgSegment);
    free(pThis->pszDir);
    free(pThis->pZipBuf);
    free(pThis->pszCurrFName);
    free(pThis->pszFName);
...
static rsRetVal strmCheckNextOutputFile(strm_t *pThis) {
    ...
    if (pThis->fd == -1 || pThis->sType != STREAMTYPE_FILE_CIRCULAR) FINALIZE;
...
#endif
static rsRetVal tryTTYRecover(strm_t *pThis, int err) {
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, strm);
#ifndef __FreeBSD__
    if (err == ERR_TTYHUP) {
 ...
 */
static rsRetVal ATTR_NONNULL(1, 2, 3) doWriteCall(strm_t *pThis, uchar *pBuf, size_t *pLenBuf) {
    ssize_t lenBuf;
    ...
    do {
        ...
        if (pThis->bIsTTY && !pThis->iZipLevel && !pThis->cryprov) {
            ...
            if (pNl == pWriteBuf) {
                ...
                iWritten = write(pThis->fd, pWriteBuf, pNl ? pNl - pWriteBuf : lenBuf);
        } else
#endif /* __FreeBSD__ */
        ...
        if (iWritten < 0) {
            ...
            if (err == EBADF) {
                DBGPRINTF(
                    ...
                    "OS error reason",
                    pThis->pszCurrFName, err, pThis->fd);
                pThis->fd = -1;
                CHKiRet(doPhysOpen(pThis));
            } else {
                if (err != EINTR) {
                    LogError(err, RS_RET_IO_ERROR,
                             "file '%s'[%d] write error - see "
 ...
 */
static rsRetVal doWriteInternal(strm_t *pThis, uchar *pBuf, const size_t lenBuf, const int bFlush) {
    DEFiRet;
...
static rsRetVal doAsyncWriteInternal(strm_t *pThis, size_t lenBuf, const int bFlushZip) {
    ...
    DBGOPRINT((obj_t *)pThis,
              "file %d(%s) doAsyncWriteInternal at begin: "
              "iCnt %d, iEnq %d, bFlushZip %d\n",
              pThis->fd, getFileDebugName(pThis), pThis->iCnt, pThis->iEnq, bFlushZip);
    /* the -1 below is important, because we need one buffer for the main thread! */
    while (pThis->iCnt >= STREAM_ASYNC_NUMBUFS - 1) d_pthread_cond_wait(&pThis->notFull, &pThis->mut);
    ...
    DBGOPRINT((obj_t *)pThis,
              "file %d(%s) doAsyncWriteInternal at exit: "
              "iCnt %d, iEnq %d, bFlushZip %d\n",
              pThis->fd, getFileDebugName(pThis), pThis->iCnt, pThis->iEnq, bFlushZip);
 ...
 */
static rsRetVal strmSchedWrite(strm_t *pThis, uchar *pBuf, size_t lenBuf, const int bFlushZip) {
    DEFiRet;
...
static void *asyncWriterThread(void *pPtr) {
    ...
    while (1) { /* loop broken inside */
        while (pThis->iCnt == 0) {
            DBGOPRINT((obj_t *)pThis,
                      "file %d(%s) asyncWriterThread new iteration, "
                      "iCnt %d, bTimedOut %d, iFlushInterval %d\n",
                      pThis->fd, getFileDebugName(pThis), pThis->iCnt, bTimedOut, pThis->iFlushInterval);
            ...
            if (pThis->bDoTimedWait) {
                ...
                if ((err = pthread_cond_timedwait(&pThis->notEmpty, &pThis->mut, &t)) != 0) {
                    ...
                    if (err != ETIMEDOUT) {
                        ...
                        rs_strerror_r(err, errStr, sizeof(errStr));
                        DBGPRINTF(
                            "stream async writer timeout with error (%d): %s - "
                            "ignoring\n",
                            err, errStr);
                    }
                }
        ...
        DBGOPRINT((obj_t *)pThis,
                  "file %d(%s) asyncWriterThread awoken, "
                  "iCnt %d, bTimedOut %d\n",
                  pThis->fd, getFileDebugName(pThis), pThis->iCnt, bTimedOut);
        ...
        if (pThis->iCnt < STREAM_ASYNC_NUMBUFS) {
            ...
            if (pThis->iCnt == 0) pthread_cond_broadcast(&pThis->isEmpty);
        }
    }
    /* Not reached */
...
#endif
static rsRetVal syncFile(strm_t *pThis) {
    int ret;
    DEFiRet;

    if (pThis->bIsTTY) FINALIZE; /* TTYs can not be synced */
    ...
    if (pThis->fdDir != -1) {
        if (fsync(pThis->fdDir) != 0) DBGPRINTF("stream/syncFile: fsync returned error, ignoring\n");
    }
...
static rsRetVal strmPhysWrite(strm_t *pThis, uchar *pBuf, size_t lenBuf) {
    ...
    if (pThis->pUsrWCntr != NULL) *pThis->pUsrWCntr += iWritten;
 ...
 * rgerhards, 2009-06-04
 */
static rsRetVal doZipWrite(strm_t *pThis, uchar *pBuf, size_t lenBuf, const int bFlush) {
    if (pThis->compressionDriver == STRM_COMPRESS_ZSTD) {
...
static rsRetVal strmFlushInternal(strm_t *pThis, int bFlushZip) {
    ...
    DBGOPRINT((obj_t *)pThis, "strmFlushinternal: file %d(%s) flush, buflen %ld%s\n", pThis->fd,
              getFileDebugName(pThis), (long)pThis->iBufPtr, (pThis->iBufPtr == 0) ? " (no need to flush)" : "");
 ...
 */
static rsRetVal ATTR_NONNULL() strmSeek(strm_t *pThis, const off64_t offs) {
    DEFiRet;
    ...
    if (i != offs) {
        LogError(errno, RS_RET_IO_ERROR,
                 "file %s: unexpected error seeking to "
...
static rsRetVal ATTR_NONNULL(1, 2)
    ...
    do {
        ...
        if (iWrite > lenBuf) iWrite = lenBuf;
...
static rsRetVal ATTR_NONNULL() strmSetbDeleteOnClose(strm_t *const pThis, const int val) {
    ISOBJ_TYPE_assert(pThis, strm);
...
static rsRetVal ATTR_NONNULL() strmSetiMaxFiles(strm_t *const pThis, const int iNewVal) {
    ISOBJ_TYPE_assert(pThis, strm);
...
static rsRetVal ATTR_NONNULL() strmSetFileNotFoundError(strm_t *const pThis, const int pFileNotFoundError) {
    ISOBJ_TYPE_assert(pThis, strm);
 ...
 * rgerhards, 2008-01-09
 */
static rsRetVal strmSetFName(strm_t *pThis, uchar *pszName, size_t iLenName) {
    DEFiRet;

    assert(pThis != NULL);
    assert(pszName != NULL);

    if (iLenName < 1) ABORT_FINALIZE(RS_RET_FILE_PREFIX_MISSING);

    if (pThis->pszFName != NULL) free(pThis->pszFName);

    if ((pThis->pszFName = malloc(iLenName + 1)) == NULL) ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
 ...
 * rgerhards, 2008-01-09
 */
static rsRetVal strmSetDir(strm_t *pThis, uchar *pszDir, size_t iLenDir) {
    DEFiRet;

    assert(pThis != NULL);
    assert(pszDir != NULL);

    if (iLenDir < 1) ABORT_FINALIZE(RS_RET_FILE_PREFIX_MISSING);
 ...
 */
static rsRetVal strmDup(strm_t *const pThis, strm_t **ppNew) {
    strm_t *pNew = NULL;
    DEFiRet;
...
finalize_it:
    if (pNew != NULL) strmDestruct(&pNew);

    RETiRet;
}


static rsRetVal SetCompressionWorkers(strm_t *const pThis, int num_wrkrs) {
    ISOBJ_TYPE_assert(pThis, strm);
    if (num_wrkrs < 0) num_wrkrs = 1;
 ...
 */
static rsRetVal strmSetWCntr(strm_t *pThis, number_t *pWCnt) {
    DEFiRet;

    ISOBJ_TYPE_assert(pThis, strm);

    if (pWCnt != NULL) *pWCnt = 0;
 ...
 */
static rsRetVal strmGetCurrOffset(strm_t *pThis, int64 *pOffs) {
    DEFiRet;

...
int rsksiInit(__attribute__((unused)) char *usragent)
{
	return 0;
}
...
readStateFile(ksifile ksi)
{
	...
	if (read(fd, ksi->x_prev->data, ksi->x_prev->len) != (ssize_t)ksi->x_prev->len) {
...
int hash_r_ksi(ksifile ksi, KSI_DataHash **r, const uchar *rec, const size_t len)
{
	int ret = 0, rgt;
...
int hash_node_ksi(ksifile ksi, KSI_DataHash **node, KSI_DataHash *m, KSI_DataHash *rec,
    uint8_t level)
{
	int r = 0, rgt;
	uchar concatBuf[16 * 1024];
...
int sigblkAddRecordKSI(ksifile ksi, const uchar *rec, const size_t len)
{
	...
	if (ksi == NULL || ksi->disabled)
		goto done;
	if ((ret = hash_m_ksi(ksi, &m)) != 0)
		goto done;
	if ((ret = hash_r_ksi(ksi, &r, rec, len)) != 0)
		goto done;
	if (ksi->nRecords == 0)
		tlvWriteBlockHdrKSI(ksi);
	if (ksi->bKeepRecordHashes)
		tlvWriteHashKSI(ksi, 0x0902, r);
	if ((ret = hash_node_ksi(ksi, &x, m, r, 1)) != 0)
		goto done; /* hash leaf */
...
int sigblkFinishKSI(ksifile ksi)
{
	...
	for (j = 0; j < ksi->nRoots; ++j) {
		if (root == NULL) {
			...
			if (ret != 0)
				goto done; /* checks hash_node_ksi() result! */
		}
	}
	signIt(ksi, root);

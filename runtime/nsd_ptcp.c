 ...
 */
static rsRetVal SetMode(nsd_t __attribute__((unused)) * pNsd, int mode) {
    DEFiRet;
    if (mode != 0) {
...
static rsRetVal SetCheckExtendedKeyUsage(nsd_t __attribute__((unused)) * pNsd, int ChkExtendedKeyUsage) {
    ...
    if (ChkExtendedKeyUsage != 0) {
        LogError(0, RS_RET_VALUE_NOT_SUPPORTED,
                 ...
                 "not supported by ptcp netstream driver",
                 ChkExtendedKeyUsage);
 ...
 * alorbach, 2019-12-20
 */
static rsRetVal SetTlsVerifyDepth(nsd_t __attribute__((unused)) * pNsd, int verifyDepth) {
...
static rsRetVal SetTlsCRLFile(nsd_t __attribute__((unused)) * pNsd, const uchar *const pszFile) {
    ...
    if (pszFile != NULL) {
        LogError(0, RS_RET_VALUE_NOT_SUPPORTED,
                 "error: CRL File setting not supported by "
                 "ptcp netstream driver - value %s",
                 pszFile);
        ABORT_FINALIZE(RS_RET_VALUE_NOT_SUPPORTED);
    }
...
static rsRetVal SetTlsKeyFile(nsd_t __attribute__((unused)) * pNsd, const uchar *const pszFile) {
    ...
    if (pszFile != NULL) {
        LogError(0, RS_RET_VALUE_NOT_SUPPORTED,
                 "error: TLS Key File setting not supported by "
                 "ptcp netstream driver");
        ABORT_FINALIZE(RS_RET_VALUE_NOT_SUPPORTED);
    }
finalize_it:
...
static rsRetVal SetTlsCertFile(nsd_t __attribute__((unused)) * pNsd, const uchar *const pszFile) {
    DEFiRet;
    if (pszFile != NULL) {
        LogError(0, RS_RET_VALUE_NOT_SUPPORTED,
                 "error: TLS Cert File setting not supported by "
                 "ptcp netstream driver");
        ABORT_FINALIZE(RS_RET_VALUE_NOT_SUPPORTED);
    }
 ...
 * rgerhards, 2008-05-17
 */
static rsRetVal SetPermPeers(nsd_t __attribute__((unused)) * pNsd,
                             permittedPeers_t __attribute__((unused)) * pPermPeers) {
    DEFiRet;
...
static void get_socket_info(const int sockfd, char *const connInfo) {
    ...
    snprintf(connInfo, TCPSRV_CONNINFO_SIZE, "from %s:%s to %s:%s", remote_ip_str, remote_port_str, local_ip_str,
             local_port_str);
}
...
static rsRetVal AcceptConnReq(nsd_t *pNsd, nsd_t **ppNew, char *const connInfo) {
    ...
    if (iNewSock < 0) {
        ...
        LogMsg(errno, RS_RET_ACCEPT_ERR, LOG_WARNING, "nds_ptcp: error accepting connection on socket %d", pThis->sock);
...
finalize_it:
    if (iRet != RS_RET_OK) {
        if (pNew != NULL) nsd_ptcpDestruct(&pNew);

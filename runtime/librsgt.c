...
void rsgtsetErrFunc(gtctx ctx, void (*func)(void *, uchar *), void *usrptr)
{
	ctx->usrptr = usrptr;
	ctx->errFunc = func;
}
...
int tlvFlush(gtfile gf)
{
	return (gf->tlvIdx == 0) ? 0 : tlvbufPhysWrite(gf);
}

int tlvWriteHash(gtfile gf, uint16_t tlvtype, GTDataHash *rec)
{
	unsigned tlvlen;
	int r;
...
int tlvWriteBlockHdr(gtfile gf)
{
	unsigned tlvlen;
	int r;
...
int tlvWriteBlockSig(gtfile gf, uchar *der, uint16_t lenDer)
{
	unsigned tlvlen;
...
readStateFile(gtfile gf)
{
	...
	if (read(fd, gf->x_prev->data, gf->x_prev->len) != (ssize_t)gf->x_prev->len) {
 ...
 * be read from file.
 */
int tlvOpen(gtfile gf, char *hdr, unsigned lenHdr)
{
	int r = 0;
	struct stat stat_st;
...
int hash_r(gtfile gf, GTDataHash **r, const uchar *rec, const size_t len)
{
	int ret = 0, rgt;
...
int hash_node(gtfile gf, GTDataHash **node, GTDataHash *m, GTDataHash *rec,
    uint8_t level)
{
	int r = 0, rgt;
	uchar concatBuf[16 * 1024];
...
int sigblkAddRecord(gtfile gf, const uchar *rec, const size_t len)
{
	...
	if (gf == NULL || gf->disabled)
		goto done;
	if ((ret = hash_m(gf, &m)) != 0)
		goto done;
	if ((ret = hash_r(gf, &r, rec, len)) != 0)
		goto done;
	if (gf->nRecords == 0)
		tlvWriteBlockHdr(gf);
	if (gf->bKeepRecordHashes)
		tlvWriteHash(gf, 0x0902, r);
	if ((ret = hash_node(gf, &x, m, r, 1)) != 0)
		goto done; /* hash leaf */
...
int sigblkFinish(gtfile gf)
{
	...
	for (j = 0; j < gf->nRoots; ++j) {
		if (root == NULL) {
			...
			if (ret != 0)
				goto done; /* checks hash_node() result! */
		}
	}
	if ((ret = timestampIt(gf, root)) != 0)
		goto done;

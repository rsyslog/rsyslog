...
static rsRetVal doGetSize(uchar **pp, rsRetVal (*pSetHdlr)(void *, int64), void *pVal) {
    ...
    switch (**pp) {
        ...
        case 'g':
            ...
            break;
        case 't':
            i *= (int64)1024 * 1024 * 1024 * 1024;
            ++(*pp);
            break; /* tera */
        case 'p':
            i *= (int64)1024 * 1024 * 1024 * 1024 * 1024;
            ++(*pp);
            break; /* peta */
        case 'e':
            i *= (int64)1024 * 1024 * 1024 * 1024 * 1024 * 1024;
            ++(*pp);
            break; /* exa */
        ...
        case 'T':
            i *= (int64)1000 * 1000000000;
            ++(*pp);
            break; /* tera */
        case 'P':
            i *= (int64)1000000 * 1000000000;
            ++(*pp);
            break; /* peta */
        case 'E':
            i *= (int64)1000000000 * 1000000000;
            ++(*pp);
            break; /* exa */
        default:
...
static rsRetVal doFileCreateMode(uchar **pp, rsRetVal (*pSetHdlr)(void *, uid_t), void *pVal) {
     ...
     */
    if (!((*p == '0') && (*(p + 1) && *(p + 1) >= '0' && *(p + 1) <= '7') &&
          (*(p + 2) && *(p + 2) >= '0' && *(p + 2) <= '7') && (*(p + 3) && *(p + 3) >= '0' && *(p + 3) <= '7'))) {
...
static rsRetVal doGetGID(uchar **pp, rsRetVal (*pSetHdlr)(void *, uid_t), void *pVal) {
    ...
    if (pgBuf == NULL) {
        if (err != 0) {
            LogError(err, RS_RET_NOT_FOUND, "Query for group '%s' resulted in an error", szName);
        } else {
...
static rsRetVal doBinaryOptionLine(uchar **pp, rsRetVal (*pSetHdlr)(void *, int), void *pVal) {
    ...
    if ((iOption = doParseOnOffOption(pp)) == -1) return RS_RET_ERR; /* nothing left to do */
 ...
 */
static rsRetVal getWord(uchar **pp, cstr_t **ppStrB) {
    DEFiRet;
    uchar *p;
...
static rsRetVal doGetWord(uchar **pp, rsRetVal (*pSetHdlr)(void *, uchar *), void *pVal) {
    ...
    if (pSetHdlr == NULL) {
        /* we should set value directly to var */
        if (*((uchar **)pVal) != NULL) free(*((uchar **)pVal)); /* free previous entry */
 ...
 */
static rsRetVal doSyslogName(uchar **pp, rsRetVal (*pSetHdlr)(void *, int), void *pVal, syslogName_t *pNameTable) {
    DEFiRet;
    cstr_t *pStrB;
    int iNewVal;
...
finalize_it:
    if (pStrB != NULL) rsCStrDestruct(&pStrB);
 ...
 * rgerhards, 2008-02-14
 */
static rsRetVal doFacility(uchar **pp, rsRetVal (*pSetHdlr)(void *, int), void *pVal) {
    DEFiRet;
...
static rsRetVal doGoneAway(__attribute__((unused)) uchar **pp,
 ...
 * rgerhards, 2008-02-14
 */
static rsRetVal doSeverity(uchar **pp, rsRetVal (*pSetHdlr)(void *, int), void *pVal) {
    DEFiRet;
 ...
 */
static rsRetVal cslchSetEntry(
    cslCmdHdlr_t *pThis, ecslCmdHdrlType eType, rsRetVal (*pHdlr)(), void *pData, int *permitted) {
    assert(pThis != NULL);
...
static rsRetVal cslchCallHdlr(cslCmdHdlr_t *pThis, uchar **ppConfLine) {
    ...
    /* CRITICAL FIX for macOS 14+ ARM64 crash: Replace unsafe function pointer casting
     * with direct function calls to avoid undefined behavior on ARM64 calling conventions.
     * ThreadSanitizer identified this as the root cause of BUS errors. */
    switch (pThis->eType) {
 ...
 */
static rsRetVal cslcAddHdlr(
    cslCmd_t *pThis, ecslCmdHdrlType eType, rsRetVal (*pHdlr)(), void *pData, void *pOwnerCookie, int *permitted) {
    DEFiRet;
...
finalize_it:
    if (iRet != RS_RET_OK) {
        if (pHdlr != NULL) cslchDestruct(pCmdHdlr);
    }
 ...
 */
rsRetVal regCfSysLineHdlr2(const uchar *pCmdName,
                           int bChainingPermitted,
                           ecslCmdHdrlType eType,
                           rsRetVal (*pHdlr)(),
                           void *pData,
                           void *pOwnerCookie,
                           int *permitted) {
...
rsRetVal regCfSysLineHdlr(const uchar *pCmdName,
                          int bChainingPermitted,
                          ecslCmdHdrlType eType,
                          rsRetVal (*pHdlr)(),
                          void *pData,
...
rsRetVal processCfSysLineCommand(uchar *pCmdName, uchar **p) {
    ...
    if (iRet == RS_RET_NOT_FOUND) {
        LogError(0, RS_RET_NOT_FOUND,
                 "invalid or yet-unknown config file command '%s' - "

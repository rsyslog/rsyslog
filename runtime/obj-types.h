    ...
    #define ISOBJ_TYPE_assert(pObj, objType)                                                                           \
        do {                                                                                                           \
            ...
            if (strcmp((char *)(((obj_t *)pObj)->pObjInfo->pszID), #objType)) {                                        \
                dbgprintf(                                                                                             \
                    "%s:%d ISOBJ assert failure: invalid object type, expected '%s' "                                  \
                    "actual '%s', cookie: %X\n",                                                                       \
                    __FILE__, __LINE__, #objType, (((obj_t *)pObj)->pObjInfo->pszID), ((obj_t *)(pObj))->iObjCooCKiE); \
                fprintf(stderr,                                                                                        \
                        "%s:%d ISOBJ assert failure: invalid object type, expected '%s' "                              \
                        "actual '%s', cookie: %X\n",                                                                   \
                        __FILE__, __LINE__, #objType, (((obj_t *)pObj)->pObjInfo->pszID),                              \
                        ((obj_t *)(pObj))->iObjCooCKiE);                                                               \
                fflush(stderr);                                                                                        \
    ...
    #define PTR_ASSERT_CHK(_ptr, _type)                                             \
        do {                                                                        \
            ...
            if (_ptr->_Assert_type != _type) {                                      \
                dbgprintf(                                                          \
                    "%s:%d PTR_ASSERT_CHECK failure: invalid pointer type %x, "     \
                    "expected %x\n",                                                \
...
#define PROTOTYPEpropSetMethPTR(obj, prop, dataType) rsRetVal obj##Set##prop(obj##_t *pThis, dataType *)
...
#define PROTOTYPEpropSetMeth(obj, prop, dataType) rsRetVal obj##Set##prop(obj##_t *pThis, dataType pVal)
 ...
 */
#define PROTOTYPEobjDestruct(OBJ) rsRetVal OBJ##Destruct(OBJ##_t __attribute__((unused)) * *ppThis)
...
#define CODESTARTobjQueryInterface(obj) assert(pIf != NULL);

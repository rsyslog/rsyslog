...
{
	int r = RSGTE_IO;
	if (fwrite(rec->hdr, (size_t)rec->lenHdr, 1, fp) != 1)
		goto done;
	if (fwrite(rec->data, (size_t)rec->tlvlen, 1, fp) != 1)
		goto done;
	r = 0;
done:
	return r;
...
rsksi_tlvDecodeSUBREC(tlvrecord_t *rec, uint16_t *stridx, tlvrecord_t *newrec)
{
	...
	if (rec->tlvlen == *stridx) {
		r = RSGTE_LEN;
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvDecodeSUBREC:\t\t "
			       "break #1\n");
		goto done;
	}
	c = rec->data[(*stridx)++];
...
int rsksi_tlvDecodeIMPRINT(tlvrecord_t *rec, imprint_t **imprint)
{
	...
	*imprint = imp;
	...
	if (r == 0) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvDecodeIMPRINT:\t\t returned %d TLVType=%4.4x, "
			       "TLVLen=%d, HashID=%d\n",
			    r, rec->tlvtype, rec->tlvlen, imp->hashID);
		if (rsksi_read_debug)
			outputHash(stdout, "debug: rsksi_tlvDecodeIMPRINT:\t\t hash: ",
			    imp->data, imp->len, 1);
	} else {
		/* Free memory on FAIL!*/
...
rsksi_tlvDecodeREC_HASH(tlvrecord_t *rec, uint16_t *strtidx, imprint_t *imp)
{
	...
	if (subrec.tlvlen != 1 + hashOutputLengthOctetsKSI(imp->hashID)) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvDecodeREC_HASH:\t\t FAIL on subrec.tlvtype %4.4x "
			       "subrec.tlvlen = %d\n",
	...
	if (r == 0) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvDecodeREC_HASH:\t\t returned %d TLVType=%4.4x, "
			       "TLVLen=%d\n",
...
rsksi_tlvDecodeLEVEL_CORR(tlvrecord_t *rec, uint16_t *strtidx, uint8_t *levelcorr)
{
	...
	if (!(subrec.tlvtype == 0x01 && subrec.tlvlen == 1)) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvDecodeLEVEL_CORR:\t FAIL on subrec.tlvtype "
	...
	*levelcorr = subrec.data[0];
	...
	if (r == 0) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvDecodeLEVEL_CORR:\t returned %d TLVType=%4.4x, "
			       "TLVLen=%d\n",
			    r, rec->tlvtype, rec->tlvlen);
	} else
...
rsksi_tlvDecodeHASH_STEP(tlvrecord_t *rec, uint16_t *pstrtidx, block_hashstep_t **blhashstep)
{
	...
	*blhashstep = hashstep;
	...
	if (r == 0) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvDecodeHASH_STEP:\t returned %d, tlvtype "
			       "%4.4x\n",
...
rsksi_tlvDecodeHASH_CHAIN(tlvrecord_t *rec, block_hashchain_t **blhashchain)
{
	...
	*blhashchain = hashchain;
	...
	if (r == 0) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvDecodeHASH_CHAIN:\t returned %d TLVType=%4.4x, "
			       "TLVLen=%d\n",
			    r, rec->tlvtype, rec->tlvlen);
	} else {
		/* Free memory on FAIL!*/
...
rsksi_tlvDecodeBLOCK_SIG(tlvrecord_t *rec, block_sig_t **blocksig)
{
	...
	*blocksig = bs;
	...
	if (r == 0) {
		if (rsksi_read_debug)
			printf("debug: tlvDecodeBLOCK_SIG:\t\t returned %d, tlvtype %4.4x, recCount "
			       "%ju\n",
...
rsksi_tlvrdRecHash(FILE *fp, FILE *outfp, imprint_t **imp)
{
	...
	if ((r = rsksi_tlvrd(fp, &rec, imp)) != 0)
		goto done;
	if (rec.tlvtype != 0x0902) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvrdRecHash:\t\t\t expected tlvtype 0x0902, "
			       "but was %4.4x\n",
			    rec.tlvtype);
	...
	if (outfp != NULL) {
		if ((r = rsksi_tlvwrite(outfp, &rec)) != 0)
			goto done;
	}
	r = 0;
done:
...
rsksi_tlvrdTreeHash(FILE *fp, FILE *outfp, imprint_t **imp)
{
	...
	if (rec.tlvtype != 0x0903) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvrdTreeHash:\t\t expected tlvtype 0x0903, "
			       "but was %4.4x\n",
			    rec.tlvtype);
...
rsksi_tlvrdVrfyBlockSig(FILE *fp, block_sig_t **bs, tlvrecord_t *rec)
{
	...
	if ((r = rsksi_tlvrd(fp, rec, bs)) != 0)
		goto done;
	if (rec->tlvtype != 0x0904) {
		if (rsksi_read_debug)
			printf("debug: rsksi_tlvrdVrfyBlockSig:\t expected tlvtype 0x0904, but "
			       "was %4.4x\n",
			    rec->tlvtype);
 ...
 * @returns 0 if ok, something else otherwise
 */
int rsksi_getBlockParams(FILE *fp, uint8_t bRewind, block_sig_t **bs,
...
int rsksi_getExcerptBlockParams(FILE *fp, uint8_t bRewind, block_sig_t **bs, block_hdr_t **bh)
{
	...
	*bs = NULL;
	...
	if (*bs != NULL) {
		if (r == RSGTE_EOF) {
			if (rsksi_read_debug)
				printf("debug: rsksi_getExcerptBlockParams:\t Reached END of FILE\n");
			r = RSGTE_SUCCESS;
		}
	} else {
		goto done2;
...
int rsksi_vrfy_nextRec(ksifile ksi, FILE *sigfp, FILE *nsigfp, unsigned char *rec, size_t len, ksierrctx_t *ectx)
{
	...
	for (j = 0; j < ksi->nRoots; ++j) {
		if (ksi->roots_valid[j] == 0) {
			...
			if (ksi->bKeepTreeHashes) {
				...
				if (r != 0)
					goto done; /* mem leak ok, we terminate! */
			}
	...
	if (rsksi_read_debug)
		printf("debug: rsksi_vrfy_nextRec:\t\t returned %d\n", r);
	return r;
}

/* Helper function to verifiy the next record in the signature file */
int rsksi_vrfy_nextRecExtract(ksifile ksi, FILE *sigfp, FILE *nsigfp, unsigned char *rec, size_t len, ksierrctx_t *ectx,
    block_hashchain_t *hashchain, int storehashchain)
{
	...
	if (storehashchain == 1) {
		...
		if (hashchain->rec_hash.data == NULL) {
			...
			if (rsksi_read_debug)
				outputHash(stdout, "debug: rsksi_vrfy_nextRecExtract:\t RECORD Hash:"
						   " \t\t",
			...
			if (rsksi_read_debug)
				outputHash(stdout, "debug: rsksi_vrfy_nextRecExtract:\t RIGHT Hash:"
						   " \t\t",
	...
	for (j = 0; j < ksi->nRoots; ++j) {
		if (ksi->roots_valid[j] == 0) {
			...
			if ((j + 1) == hashchain->level) {
				...
				if (rsksi_read_debug)
					printf("debug: rsksi_vrfy_nextRecExtract:\t NEXT Level=%d\n", hashchain->level);
			}
	...
	if (recHash != NULL)
		KSI_DataHash_free(recHash);
	if (rsksi_read_debug)
		printf("debug: rsksi_vrfy_nextRecExtract:\t returned %d\n", r);
	return r;
}
...
int rsksi_vrfy_nextHashChain(ksifile ksi, block_sig_t *bs, FILE *sigfp, unsigned char *rec, size_t len, ksierrctx_t *ectx)
{
	...
	if ((r = rsksi_tlvrd(sigfp, &tlvrec, &obj)) != 0)
		goto done;
	...
	if (rsksi_read_debug)
		outputKSIHash(stdout, "debug: rsksi_vrfy_nextHashChain:\t Line Hash.:.............: ",
	...
	if (rsksi_read_debug)
		outputKSIHash(stdout, "debug: rsksi_vrfy_nextHashChain:\t HashChain Record Hash...: ",
	...
	if (ksistate != KSI_OK) {
		...
		if (rsksi_read_debug)
			printf("debug: rsksi_vrfy_nextHashChain:\t KSI_Signature_parse was successfull\n");
	}
	...
	if (ksistate != KSI_OK) {
		...
		if (rsksi_read_debug)
			printf("debug: rsksi_vrfy_nextHashChain:\t KSI_Signature_verify was successfull\n");
	}
	...
	if (ksistate != KSI_OK) {
		...
		if (rsksi_read_debug)
			printf("debug: rsksi_vrfy_nextHashChain:\t KSI_Signature_verifyDataHash was successfull\n");
		if (rsksi_read_showVerified)
	...
	if (root_hash != NULL)
		KSI_DataHash_free(root_hash);
	if (line_hash != NULL)
		KSI_DataHash_free(line_hash);
	if (rec_hash != NULL)
		KSI_DataHash_free(rec_hash);
	if (hashchain != NULL)
		rsksi_objfree(0x0907, hashchain);

	if (rsksi_read_debug)
		printf("debug: rsksi_vrfy_nextHashChain:\t returned %d\n", r);
	return r;
}
...
int verifySigblkFinishChain(ksifile ksi, block_hashchain_t *hashchain, KSI_DataHash **pRoot, ksierrctx_t *ectx)
{
	...
	if (ksi->nRecords == 0) {
		if (rsksi_read_debug)
			printf("debug: verifySigblkFinishChain:\t\t Error, No records found! %d\n", r);
		goto done;
	}
	...
	for (j = 0; j < ksi->nRoots; ++j) {
		if (root == NULL) {
			...
			if (rsksi_read_debug)
				printf("debug: verifySigblkFinishChain:\t\t Level=%d\n", j);
			if (j + 1 >= hashchain->level) {
				if (hashchain->direction == 0x03) { /*RIGHT*/
					hashstep = rsksiHashstepFromKSI_DataHash(ksi, ksi->roots_hash[j]);
					if (hashstep == NULL) {
					...
					if (rsksi_read_debug)
						outputHash(stdout, "debug: verifySigblkFinishChain:\t\t RIGHT "
								   "Hash: \t\t",
						    hashstep->sib_hash.data, hashstep->sib_hash.len,
						    ectx->verbose);
				} else { /*LEFT*/
...
int verifySigblkFinish(ksifile ksi, KSI_DataHash **pRoot)
{
	...
	if (ksi->nRecords == 0) {
		if (rsksi_read_debug)
			printf("debug: verifySigblkFinish:\t\t no records!!!%d\n", r);
		goto done;
	}
...
int verifyBLOCK_HDRKSI(FILE *sigfp, FILE *nsigfp, tlvrecord_t *tlvrec)
{
	int r;
	block_hdr_t *bh = NULL;
	if ((r = rsksi_tlvrd(sigfp, tlvrec, &bh)) != 0)
		goto done;
	...
	if (nsigfp != NULL)
		if ((r = rsksi_tlvwrite(nsigfp, tlvrec)) != 0)
			goto done;
done:
	if (bh != NULL)
 ...
 */
int verifyBLOCK_SIGKSI(block_sig_t *bs, ksifile ksi, FILE *sigfp, FILE *nsigfp,
    uint8_t bExtend, KSI_DataHash *ksiHash, ksierrctx_t *ectx)
{
	...
	if (file_bs->sig.der.data == NULL || file_bs->sig.der.len == 0) {
		if (rsksi_read_debug)
			printf("debug: verifyBLOCK_SIGKSI:\t\t KSI signature missing\n");
		if (bExtend)
			goto skip_ksi;
		else {
			r = RSGTE_MISS_KSISIG;
...
skip_ksi:
	if (bExtend)
		if ((r = rsksi_extendSig(sig, ksi, &rec, ectx)) != 0)
			goto done;

	if (nsigfp != NULL) {
		if ((r = rsksi_tlvwrite(nsigfp, &rec)) != 0)
			goto done;
	}
	r = 0;
done:
	...
	while (1) { /* we will err out on EOF */
		...
		if (rRead == 0 /*|| rRead == RSGTE_EOF*/) {
			...
			case 0x0901:
				...
				if ((r = rsksi_tlvwrite(newsigfp, &rec)) != 0)
					goto done;
...
int rsksi_WriteHashChain(FILE *newsigfp, block_hashchain_t *hashchain, int verbose)
{
	...
	if (rsksi_read_debug)
		printf("debug: rsksi_WriteHashChain:\t\t tlvlen=%d \n", tlvlen);
...
donedecode:
	if (r != 0)
		printf("debug: rsksi_WriteHashChain:\t\t failed to write with error %d\n", r);
done:
	if (rsksi_read_debug)
		printf("debug: rsksi_WriteHashChain:\t\t returned %d\n", r);
	return r;
}
...
donedecode:
	if (r != 0)
		printf("debug: rsksi_ExtractBlockSignature:\t\t failed to write with error %d\n", r);
	if (rsksi_read_debug)
		printf("debug: ExtractBlockSignature:\t\t returned %d\n", r);
	return r;
}

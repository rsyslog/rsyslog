...
struct instanceConf_s {
	...
	int bFormatMessages; /* Message received should be reformatted */
	cstr_t *ppCStr;
...
static struct cnfparamdescr inppdescr[] = {
	...
	{ "maxmessagesize", eCmdHdlrPositiveInt, 0 },
	{ "formatsessages", eCmdHdlrBinary, 0 },
...
static rsRetVal openPipe(instanceConf_t *pInst)
{
	...
	FD_SET(pInst->fdPipeFromChild, &rfds);
	nfds = (nfds > pInst->fdPipeFromChild) ?
			nfds : pInst->fdPipeFromChild+1;
...
static rsRetVal enqLine(instanceConf_t *const __restrict__ pInst)
{
	...
	if (!pInst->bFormatMessages) {
		pMsg->msgFlags = pMsg->msgFlags | KEEP_FORMAT;
	}
...
CODESTARTnewInpInst
	...
	for(i = 0 ; i < inppblk.nParams ; ++i) {
		...
		} else if(!strcmp(inppblk.descr[i].name, "formatmessages")) {
			pInst->bFormatMessages = pvals[i].val.d.n;

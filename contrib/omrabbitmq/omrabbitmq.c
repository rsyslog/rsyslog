...
static void init_recover(recover_t *fb, char *str) {
    ...
    fb->graceful_interval = (value[3]) ? value[3] : (fb->return_check_interval * 10) - fb->half_return_check_interval;
...
static int amqp_authenticate(wrkrInstanceData_t *self, amqp_connection_state_t a_conn) {
    ...
    if (ret.reply_type != AMQP_RESPONSE_NORMAL) {
        LogError(0, RS_RET_RABBITMQ_LOGIN_ERR,
                 "omrabbitmq module %d/%d: login to AMQP "
                 "server %s failed. (%d / %s)",
    ...
    if (amqp_get_rpc_reply(a_conn).reply_type != AMQP_RESPONSE_NORMAL) {
        LogError(0, RS_RET_RABBITMQ_CHANNEL_ERR, "omrabbitmq module %d/%d: open channel failed.", self->iidx,
                 self->widx);
        return 0;
    }
    ...
    if (self->pData->exchange_type) {
        ...
        amqp_exchange_declare(a_conn, 1, self->pData->exchange, cstring_bytes(self->pData->exchange_type), 0,
                              self->pData->durable, props);
#else
        amqp_exchange_declare(a_conn, 1, self->pData->exchange, cstring_bytes(self->pData->exchange_type), 0,
                              self->pData->durable, self->pData->auto_delete, 0, props);
#endif
        ...
        if (amqp_get_rpc_reply(a_conn).reply_type != AMQP_RESPONSE_NORMAL) {
            ...
            amqp_channel_close_t *chan_cls = (amqp_channel_close_t *)amqp_get_rpc_reply(a_conn).reply.decoded;

            if (amqp_get_rpc_reply(a_conn).reply_type == AMQP_RESPONSE_LIBRARY_EXCEPTION) {
                LogError(0, RS_RET_RABBITMQ_CHANNEL_ERR,
                         "omrabbitmq module %d/%d: disconnected while exchange declare (%d)", self->iidx, self->widx,
                         amqp_get_rpc_reply(a_conn).library_error);
                return 0;
            }

            LogError(0, RS_RET_RABBITMQ_CHANNEL_ERR, "omrabbitmq module %d/%d: exchange declare failed %.*s.",
                     self->iidx, self->widx, (int)chan_cls->reply_text.len, (char *)chan_cls->reply_text.bytes);
...
static amqp_connection_state_t tryConnection(wrkrInstanceData_t *self, server_t *server) {
    ...
    if (sockfd) {
        ...
        LogError(0, RS_RET_RABBITMQ_CHANNEL_ERR, "omrabbitmq module %d/%d: server %s port %d.", self->iidx, self->widx,
    ...
    if (retconn == AMQP_STATUS_OK && amqp_authenticate(self, a_conn)) return a_conn;
...
static void *run_connection_routine(void *arg) {
    ...
    while (go_on)  // this loop is used to reconnect on connection failure
    {
            ...
            if ((self->a_conn = tryConnection(self, &(self->serverActive->s))) != NULL) {
                ...
                if (self->serverActive->failures == 3) {
                    if (!self->serverBackup.s.host || self->serverBackup.failures == 3) {
                        LogError(0, RS_RET_RABBITMQ_CONN_ERR,
                                 "omrabbitmq module connection "
        ...
        if (self->serverActive == &self->serverBackup) self->last_failback = time(NULL);
            ...
            switch (result) {
                case AMQP_STATUS_NO_MEMORY:
                    LogError(0, RS_RET_OUT_OF_MEMORY,
                             "omrabbitmq module %d/%d: no memory "
                             ": aborting module.",
                             self->iidx, self->widx);
                ...
                case AMQP_STATUS_BAD_AMQP_DATA:
                    LogError(0, RS_RET_RABBITMQ_CONN_ERR,
                             "omrabbitmq module %d/%d: bad "
                             "data received : reconnect.",
                             self->iidx, self->widx);
                    self->connected = 0;
                    break;
                case AMQP_STATUS_SOCKET_ERROR:
                    LogError(0, RS_RET_RABBITMQ_CONN_ERR,
                             "omrabbitmq module %d/%d: Socket"
                             " error : reconnect.",
                             self->iidx, self->widx);
                    self->connected = 0;
                    break;
                case AMQP_STATUS_CONNECTION_CLOSED:
                    LogError(0, RS_RET_OUT_OF_MEMORY,
                             "omrabbitmq module %d/%d: Connection"
                             " closed : reconnect.",
                             self->iidx, self->widx);
                    self->connected = 0;
                    break;
    ...
    if (self->a_conn != NULL) {
        if (self->channel_opened) amqp_channel_close(self->a_conn, 1, 200);
...
static rsRetVal publishRabbitMQ(wrkrInstanceData_t *self,
                                amqp_bytes_t exchange,
                                amqp_bytes_t routing_key,
                                amqp_basic_properties_t *p_amqp_props,
    ...
    if (self->state != RS_RET_OK) ABORT_FINALIZE(self->state);

    if (!self->a_conn) {
        ABORT_FINALIZE(RS_RET_RABBITMQ_CONN_ERR);
    }

    if (manage_error(amqp_basic_publish(self->a_conn, 1, exchange, routing_key, 0, 0, p_amqp_props, body_bytes),
                     "amqp_basic_publish")) {
...
BEGINdoAction
    ...
    if (pWrkrData->pData->populate_properties) {
        ...
        msg2amqp_props_t prop_list[] = {{PROP_SYSLOGFACILITY_TEXT, "facility", NULL, 0},
...
BEGINcreateInstance
    ...
    if ((env_var = getenv("OMRABBITMQ_TEST")) != NULL) mode_test = atoi(env_var);
...
BEGINnewActInst
        ...
        if (!strcmp(actpblk.descr[i].name, "host")) {
            ...
            if (temp) {
                if (!strcasecmp(temp, "TRANSIENT") || !strcmp(temp, "1")) {
                    ...
                    if (!strcasecmp(temp, "PERSISTENT") || !strcmp(temp, "2")) {
                        pData->delivery_mode = 2;
                    } else {
    ...
    if (host == NULL) {
        LogError(0, RS_RET_INVALID_PARAMS,
                 "omrabbitmq module %d disabled: parameter "
    ...
    if (pData->routing_key_template &&
        tplFind(ourConf, (char *)pData->routing_key_template, strlen((char *)pData->routing_key_template)) == NULL) {
        LogError(0, RS_RET_INVALID_PARAMS,
                 "omrabbitmq module %d : template '%s'"
    ...
    if (pData->routing_key.bytes == NULL && pData->routing_key_template == NULL) {
        LogError(0, RS_RET_INVALID_PARAMS,
                 "omrabbitmq module %d disabled: "
                 "one of parameters routing_key or "
                 "routing_key_template must be specified",
                 pData->iidx);
        ABORT_FINALIZE(RS_RET_CONFIG_ERROR);
    }
    ...
    if (!pData->delivery_mode) {
        LogError(0, RS_RET_CONF_PARAM_INVLD,
                 ...
                 "TRANSIENT or PERSISTENT (default)",
                 pData->iidx);
        ABORT_FINALIZE(RS_RET_CONFIG_ERROR);
    }
    ...
    if (pData->body_template && *pData->body_template &&
        tplFind(ourConf, (char *)pData->body_template, strlen((char *)pData->body_template)) == NULL) {
        LogError(0, RS_RET_CONF_PARAM_INVLD,
                 "omrabbitmq module %d : template '%s'"
    ...
    if (pData->routing_key_template) {
        ...
        CHKiRet(
            OMSRsetEntry(*ppOMSR, 1, (uchar *)strdup((const char *)pData->routing_key_template), OMSR_NO_RQD_TPL_OPTS));
    }
    ...
    if (pData->body_template == NULL) { /* no template */
        ...
        pData->amqp_props_tpl_type.content_type =
            amqp_cstring_bytes((pData->content_type) ? pData->content_type : (char *)pData->body_template);
    } else {
        pData->idx_body_template = 0;
        pData->amqp_props_tpl_type.content_type =
            amqp_cstring_bytes((pData->content_type) ? pData->content_type : "raw");
    }
...
BEGINcreateWrkrInstance
    ...
    memcpy(&(pWrkrData->recover_policy), &(pData->recover_policy), sizeof(recover_t));
    ...
    if (pData->server2.host && *pData->server2.host) {
        ...
        memcpy(&(pWrkrData->serverBackup.s), (odd) ? &pData->server2 : &pData->server1, sizeof(server_t));
    } else {

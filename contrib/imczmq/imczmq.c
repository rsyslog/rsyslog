...
static rsRetVal addListener(instanceConf_t *iconf) {
    ...
    int rc = zsock_attach(pData->sock, (const char *)iconf->sockEndpoints, iconf->serverish);
    if (rc == -1) {
        LogError(0, NO_ERRCODE, "zsock_attach to %s failed", iconf->sockEndpoints);
        ABORT_FINALIZE(RS_RET_ERR);
    }
...
static rsRetVal rcvData(void) {
    ...
    while (which) {
        ...
        if (frame != NULL) buf = zframe_strdup(frame);
...
BEGINwillRun
    ...
    CHKiRet(prop.SetString(s_namep, UCHAR_CONSTANT("imczmq"), sizeof("imczmq") - 1));
...
static inline void std_checkRuleset_genErrMsg(__attribute__((unused)) modConfData_t *modConf, instanceConf_t *inst) {
    LogError(0, NO_ERRCODE,
             ...
             inst->pszBindRuleset, inst->sockEndpoints);
}
...
BEGINnewInpInst
    ...
    if (NULL == pvals) {
        LogError(0, RS_RET_MISSING_CNFPARAMS, "imczmq: required parameters are missing\n");
        ABORT_FINALIZE(RS_RET_MISSING_CNFPARAMS);
    }

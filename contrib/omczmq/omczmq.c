...
static struct cnfparamdescr modpdescr[] = {{"authenticator", eCmdHdlrBinary, 0},
...
static struct cnfparamdescr actpdescr[] = {
    {"endpoints", eCmdHdlrGetWord, 1},        {"socktype", eCmdHdlrGetWord, 1},
    {"sendhwm", eCmdHdlrGetWord, 0},
#if (CZMQ_VERSION_MAJOR >= 4 && ZMQ_VERSION_MAJOR >= 4 && ZMQ_VERSION_MINOR >= 2)
    {"heartbeatttl", eCmdHdlrGetWord, 0},     {"heartbeativl", eCmdHdlrGetWord, 0},
    {"heartbeattimeout", eCmdHdlrGetWord, 0}, {"connecttimeout", eCmdHdlrGetWord, 0},
#endif
    {"sendtimeout", eCmdHdlrGetWord, 0},      {"template", eCmdHdlrGetWord, 0},
    {"topics", eCmdHdlrGetWord, 0},           {"topicframe", eCmdHdlrGetWord, 0},
...
static rsRetVal initCZMQ(instanceData *pData) {
    ...
    if (!pData->sock) {
        LogError(0, RS_RET_NO_ERRCODE, "omczmq: new socket failed for endpoints: %s", pData->sockEndpoints);
    ...
    if (runModConf->authType) {
        if (!strcmp(runModConf->authType, "CURVESERVER")) {
            ...
            if (!serverCert) {
                LogError(0, NO_ERRCODE, "could not load cert %s", runModConf->serverCertPath);
            ...
            if (!clientCert) {
                LogError(0, NO_ERRCODE, "could not load cert %s", runModConf->clientCertPath);
                ABORT_FINALIZE(RS_RET_ERR);
            }
    ...
    if (rc == -1) {
        LogError(0, NO_ERRCODE, "zsock_attach to %s failed", pData->sockEndpoints);
        ABORT_FINALIZE(RS_RET_SUSPENDED);
    }
...
static rsRetVal outputCZMQ(uchar **ppString, instanceData *pData) {
    ...
    if (pData->sockType == ZMQ_PUB && pData->topics) {
        ...
        while (topic) {
            ...
            if (pData->dynaTopic) topic = (const char *)ppString[templateIndex];
    ...
    for (i = 0; i < actpblk.nParams; ++i) {
        ...
        if (!strcmp(actpblk.descr[i].name, "endpoints")) {
            ...
            if (stringType != NULL) {
                ...
                LogError(0, RS_RET_OUT_OF_MEMORY, "omczmq: out of memory");
            ...
            char topic[256];
            if (topics == NULL) {
                LogError(0, RS_RET_OUT_OF_MEMORY, "out of memory");
                ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
            }
            ...
            LogError(0, NO_ERRCODE, "omczmq: config error - '%s' is not a valid option", actpblk.descr[i].name);

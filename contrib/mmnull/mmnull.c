...
BEGINnewActInst
	...
	int i;
CODESTARTnewActInst
	DBGPRINTF("newParserInst (mmnull)\n");
	...
	if(Debug) {
		dbgprintf("parser param blk in mmnull:\n");
	...
	for(i = 0 ; i < parserpblk.nParams ; ++i) {
		...
		if(!strcmp(parserpblk.descr[i].name, "tag")) {
			...
			dbgprintf("mmnull: program error, non-handled "
...
CODESTARTdoAction
	DBGPRINTF("Message will now be managed by mmnull\n");
...
CODEmodInit_QueryRegCFSLineHdlr
	...
	DBGPRINTF("mmnull parser init called\n");
ENDmodInit

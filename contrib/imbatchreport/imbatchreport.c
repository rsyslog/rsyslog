...
DEF_IMOD_STATIC_DATA /* must be present, starts static data */
    DEFobjCurrIf(glbl) DEFobjCurrIf(ruleset) DEFobjCurrIf(datetime) DEFobjCurrIf(prop)
...
static void pollFileCancelCleanup(void *pArg) {
    ...
    if (ppInst->fd > 0) close(ppInst->fd);
    if (ppInst->pszNewFName) free(ppInst->pszNewFName);
}
...
static rsRetVal readAndSendFile(instanceConf_t *pInst, char *filename, char *fpath, struct stat *fstat) {
    ...
    if (lseek(pInst->fd, file_len - sd_buf_l, SEEK_SET) >= 0) {
        ...
        if ((t = read(pInst->fd, sd_buffer, sd_buf_l)) != sd_buf_l) {
            LogError(0, RS_RET_READ_ERR, "read end of file for structured data failed (%zu / %zu)", t, sd_buf_l);
            return RS_RET_READ_ERR;
        }

        /* let's trim the end */
        for (; sdp > sd_buffer && (*sdp == '\n' || *sdp == '\t' || *sdp == ' '); sdp--, sd_buf_l--) file_len--;
    ...
    /* Now read the file */
    msg_len = 0;
    while (msg_len < fixedModConf.max_msg_size &&
           (read_len = read(pInst->fd, read_buffer, (file_len > READ_BUFFER_LEN) ? READ_BUFFER_LEN : file_len)) > 0) {
        file_len -= read_len;
        idx = 0;
        while (read_len > 0 && msg_len < fixedModConf.max_msg_size) {
            switch (read_buffer[idx]) {
                ...
                case ' ':
                    ...
                    if (!last_is_space) fixedModConf.msg_buffer[msg_len++] = ' ';
    ...
    if ((iRet = enqMsg(pInst, pMsg)) == RS_RET_OK && (file_len > 0 || read_len > 0)) iRet = RS_RET_FILE_TOO_LARGE;
...
static void pollFile(instanceConf_t *pInst) {
    ...
    if (glob((char *)pInst->pszFollow_glob, GLOB_NOSORT, 0, &glob_res) != 0) FINALIZE;
    ...
    for (size_t i = 0; i < glob_res.gl_pathc && glbl.GetGlobalInputTermState() == 0; i++) {
        ...
        if (!stat(fpath, &fstat) && S_ISREG(fstat.st_mode)) {
            ...
            pInst->fd = open(fpath, O_NOCTTY | O_RDONLY | O_NONBLOCK | O_LARGEFILE, 0);
            ...
            if (pInst->action == action_rename || toolargeOrFailure) {
                ...
                if (rename(fpath, pInst->pszNewFName)) {
                    ...
                    LogError(0, RS_RET_STREAM_DISABLED,
                             "module stopped : was unable"
                             " to rename form %s to %s.",
                ...
                if (unlink(fpath)) {
                     ...
                     * but it keep chance to manage max files as possible
                     */
                    LogError(0, RS_RET_STREAM_DISABLED, "module stopped : unable to delete %s.", fpath);
                    pInst->must_stop = 1;
                } else
...
static rsRetVal checkInstance(instanceConf_t *inst) {
    ...
    if (r != 0) {
        eno = errno;
        rs_strerror_r(eno, errStr, sizeof(errStr));
        LogError(0, RS_RET_CONFIG_ERROR, "Configured directory can not be stated '%s': %s", dirn, errStr);
    ...
    for (s = (char *)inst->pszFollow_glob, d = dirn; *s; s++, d++) *d = (*s != '*' && *s != '?') ? *s : '~';
    *d = '\0';

    if (regexec(&inst->ff_preg, dirn, 1, matches, 0)) {
        LogError(0, RS_RET_CONFIG_ERROR, "Regex does not match globed filenames: Instance ignored to avoid loops.");
        ABORT_FINALIZE(RS_RET_CONFIG_ERROR);
    }
...
BEGINnewInpInst
    ...
    for (i = 0; i < inppblk.nParams; ++i) {
        ...
        if (!strcmp(inppblk.descr[i].name, "reports")) {
            ...
            if (inst->len_reject == 0) {
                LogError(0, RS_RET_PARAM_ERROR, "'delete' must specify TWO parameters separated by spaces or tabs !");
                ABORT_FINALIZE(RS_RET_PARAM_ERROR);
            }
    ...
    CHKiRet(ratelimitNewWithThreadMode(&inst->ratelimiter, "imbatchreport", (char *)inst->pszFollow_glob, 1));
...
finalize_it:
    CODE_STD_FINALIZERnewInpInst
        ...
        if (iRet == RS_RET_OK) addInstance(inst);
    else freeInstance(inst);
...
BEGINcheckCnf
    ...
    if (fixedModConf.root == NULL) {
        LogError(0, RS_RET_NO_LISTNERS, "no files configured to be monitored - no input will be gathered");
...
BEGINrunInput
    ...
    while (glbl.GetGlobalInputTermState() == 0) {
        ...
        for (pInst = fixedModConf.root; pInst != NULL; pInst = pInst->next) {
            if (pInst->goon) {
                if (glbl.GetGlobalInputTermState() == 1) break;
                pollFile(pInst);
        ...
        if (glbl.GetGlobalInputTermState() == 0) srSleep(fixedModConf.iPollInterval, 10);
    }
...
BEGINafterRun
    CODESTARTafterRun;
    if (fixedModConf.msg_buffer) free(fixedModConf.msg_buffer);
...
static inline void std_checkRuleset_genErrMsg(__attribute__((unused)) modConfData_t *modConf, instanceConf_t *inst) {
    LogError(0, NO_ERRCODE,
             "ruleset '%s' for %s not found - "
             "using default ruleset instead",
             inst->pszBindRuleset, inst->pszFollow_glob);
}

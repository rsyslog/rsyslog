...
enum { dst_invalid = -1, dst_stdin, dst_stdout, dst_stderr, dst_stream_type_count } docker_stream_type_t;
...
typedef rsRetVal (*submitmsg_funcptr)(struct docker_cont_logs_inst_s *pInst,
...
/* docker_cont_logs_inst_t */
static rsRetVal dockerContLogsInstNew(docker_cont_logs_inst_t **ppThis,
                                      const char *id,
                                      docker_container_info_t *container_info,
                                      SubmitMsgFuncPtr submitMsg);

static void dockerContLogsInstDestruct(docker_cont_logs_inst_t *pThis);
static rsRetVal dockerContLogsInstSetUrlById(sbool isInit,
...
static rsRetVal dockerContLogReqsGet(docker_cont_log_instances_t *pThis,
                                     docker_cont_logs_inst_t **ppContLogsInst,
                                     const char *id);
...
DEFobjCurrIf(glbl) DEFobjCurrIf(prop) DEFobjCurrIf(parser) DEFobjCurrIf(datetime) DEFobjCurrIf(statsobj)
...
static rsRetVal dockerContLogsBufWrite(docker_cont_logs_buf_t *const pThis,
                                       const uchar *const pdata,
 ...
 * debugging aide
 */
static rsRetVal dockerContLogsInstPrint(docker_cont_logs_inst_t *pThis) {
    DEFiRet;
...
static rsRetVal parseLabels(docker_cont_logs_inst_t *inst, const uchar *json) {
    DEFiRet;
    ...
    while (!fjson_object_iter_equal(&it, &itEnd)) {
        if (Debug) {
            DBGPRINTF("%s - \t%s: '%s'\n", __FUNCTION__, fjson_object_iter_peek_name(&it),
...
static rsRetVal dockerContLogsInstSetUrl(docker_cont_logs_inst_t *pThis, CURLM *curlm, const char *pUrl) {
    DEFiRet;
    ...
    if (curlm) {
        ...
        if (!runModConf->dockerApiAddr) {
            ccode = curl_easy_setopt(req->curl, CURLOPT_UNIX_SOCKET_PATH, runModConf->dockerApiUnixSockAddr);
            if (ccode != CURLE_OK) {
...
static void dockerContLogReqsDestructForHashtable(void *pData) {
...
static rsRetVal dockerContainerInfoNew(docker_container_info_t **ppThis) {
    DEFiRet;
...
static void dockerContainerInfoDestruct(docker_container_info_t *pThis) {
    if (pThis) {
        ...
        if (pThis->json_str_labels) {
            free(pThis->json_str_labels);
        }
        free(pThis);
    }
}
...
BEGINactivateCnf
    ...
    CHKiRet(statsobj.AddCounter(modStats, UCHAR_CONSTANT("curl.errors"), ctrType_IntCtr, CTR_FLAG_RESETTABLE,
                                &ctrCurlError));
...
static rsRetVal SubmitMultiLineMsg(docker_cont_logs_inst_t *pInst,
                                   docker_cont_logs_buf_t *pBufData,
                                   const uchar *pszTag,
                                   size_t len) {
    DEFiRet;
...
static size_t imdocker_container_logs_curlCB(void *data, size_t size, size_t nmemb, void *buffer) {
    ...
    if (req->data_bufs[dst_stdout]->bytes_remaining || req->data_bufs[dst_stderr]->bytes_remaining) {
        ...
        DBGPRINTF(
            "Chunk continuation, remaining bytes: type: %d, "
            "bytes remaining: %u, realsize: %u, data pos: %u\n",
            stream_type, (unsigned int)pDataBuf->bytes_remaining, (unsigned int)realsize,
            (unsigned int)pDataBuf->buf->len);
    ...
    if (get_stream_info(pdata, realsize, &stream_type, &payload_size) && is_valid_stream_type(stream_type)) {
        ...
        if (p_json_elm) {
            ...
            while (!fjson_object_iter_equal(&it, &itEnd)) {
                ...
                if (strcmp(fjson_object_iter_peek_name(&it), DOCKER_CONTAINER_ID_PARSE_NAME) == 0) {
                    containerId = fjson_object_get_string(fjson_object_iter_peek_value(&it));
                } else if (strcmp(fjson_object_iter_peek_name(&it), DOCKER_CONTAINER_NAMES_PARSE_NAME) == 0) {
                    int names_array_length = fjson_object_array_length(fjson_object_iter_peek_value(&it));
                    ...
                    containerInfo.created = fjson_object_get_int64(fjson_object_iter_peek_value(&it));
                } else if (strcmp(fjson_object_iter_peek_name(&it), DOCKER_CONTAINER_LABELS_PARSE_NAME) == 0) {
                    containerInfo.json_str_labels = (uchar *)fjson_object_get_string(fjson_object_iter_peek_value(&it));
...
static void cleanupCompletedContainerRequests(docker_cont_log_instances_t *pInstances) {
    // clean up
        ...
        if (msg->msg == CURLMSG_DONE) {
            ...
            if ((ccode = curl_easy_getinfo(pCurl, CURLINFO_PRIVATE, &szContainerId)) == CURLE_OK) {
                ...
                LogError(0, RS_RET_ERR,
                         "imdocker: private data not found "
...
static rsRetVal processAndPollContainerLogs(docker_cont_log_instances_t *pInstances) {
    DEFiRet;
    int count = 0;
...
static void *getContainersTask(void *pdata) {
...
BEGINrunInput
    ...
    CHKiRet(ratelimitNewWithThreadMode(&ratelimiter, "imdocker", NULL, 1));

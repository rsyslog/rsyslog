...
static struct cnfparamdescr inppdescr[] = {{"ulogbase", eCmdHdlrString, CNFPARAM_REQUIRED},
...
static int getFullStateFileName(uchar *pszstatefile, uchar *pszout, int ilenout) {
    ...
    /* Construct file name */
    lenout = snprintf((char *)pszout, ilenout, "%s/%s", (char *)(pszworkdir == NULL ? "." : (char *)pszworkdir),
                      (char *)pszstatefile);
 ...
 */
static rsRetVal parseMsg(smsg_t *pMsg, char *rawMsg, size_t msgLen, instanceConf_t *const __restrict__ pInst) {
    ...
    if (hour < 0 || hour > 23 || min < 0 || min > 59 || sec < 0 || sec > 59) return RS_RET_COULD_NOT_PARSE;

    host = rawMsg + ((rawMsg[10] == '.') ? 11 : 10);

    prog = memchr(host, '!', msgLen - (host - rawMsg));

    if (prog == NULL) return RS_RET_COULD_NOT_PARSE;

    prog++;

    strtData = memchr(prog, ':', msgLen - (prog - rawMsg));

    if (strtData == NULL) return RS_RET_COULD_NOT_PARSE;
    ...
    if ((size_t)(strtData - rawMsg) <= msgLen - 4 && memcmp(strtData, "ECID", 4) == 0) {
        ...
        if (ecidEnd != NULL && (size_t)(ecidEnd - rawMsg) <= msgLen - 3) {
            ...
            *text++ = ']';
            ...
            if (ret != RS_RET_OK) LogMsg(0, ret, LOG_WARNING, "Add StructuredData to message failed.");
        }
    }
    ...
    if (text != NULL) MsgSetRawMsg(pMsg, text, msgLen);
    ...
    if (ret != RS_RET_OK) LogMsg(0, ret, LOG_WARNING, "Set PROCID to message failed.");
...
static rsRetVal enqLine(instanceConf_t *const __restrict__ pInst, cstr_t *const __restrict__ cstrLine) {
    DEFiRet;
    smsg_t *pMsg;
    ...
    if (parseMsg(pMsg, (char *)rsCStrGetSzStrNoNULL(cstrLine), msgLen, pInst) != RS_RET_OK) {
        if ((ret = msgDestruct(&pMsg)) != RS_RET_OK) LogMsg(0, ret, LOG_ERR, "msgDestruct failed.");
        FINALIZE;
    }
...
static rsRetVal ATTR_NONNULL(1) openFileWithStateFile(instanceConf_t *const __restrict__ pInst) {
    ...
    DBGPRINTF(
        "deserialized state file, state file base name '%s', "
        "configured base name '%s'\n",
        pInst->pStrm->pszFName, pInst->pszUlogBaseName);
    if (ustrcmp(pInst->pStrm->pszFName, pInst->pszCurrFName)) {
        LogError(0, RS_RET_STATEFILE_WRONG_FNAME,
                 "imtuxedoulog: state file '%s' "
...
static rsRetVal openFileWithoutStateFile(instanceConf_t *const __restrict__ pInst) {
    ...
    if (pInst->pStrm != NULL) strm.Destruct(&pInst->pStrm);
...
static void persistStrmState(instanceConf_t *pInst) {
    ...
    if (lenDir > 0) CHKiRet(strm.SetDir(psSF, glbl.GetWorkDir(runModConf->pConf), lenDir));
...
finalize_it:
    ...
    if (iRet != RS_RET_OK) {
        LogError(0, iRet,
                 "imtuxedoulog: could not persist state "
...
static void pollFileReal(instanceConf_t *pInst, int *pbHadFileData, cstr_t **ppCStr) {
    ...
    while (glbl.GetGlobalInputTermState() == 0) {
        if (pInst->maxLinesAtOnce != 0 && nProcessed >= pInst->maxLinesAtOnce) break;
        CHKiRet(strm.ReadLine(pInst->pStrm, ppCStr, 0, 0, NULL, -1, NULL));
        ++nProcessed;
        if (pbHadFileData != NULL) *pbHadFileData = 1; /* this is just a flag, so set it and forget it */
...
static rsRetVal ATTR_NONNULL() lstnAdd(instanceConf_t *pInst) {
    DEFiRet;
    CHKiRet(ratelimitNewWithThreadMode(&pInst->ratelimiter, "imtuxedoulog", (char *)pInst->pszUlogBaseName, 1));
...
static void ATTR_NONNULL(1) lstnDel(instanceConf_t *pInst) {
    ...
    if (pInst->multiSub.ppMsgs != NULL) free(pInst->multiSub.ppMsgs);

    free(pInst->pszUlogBaseName);
    if (pInst->pszCurrFName != NULL) free(pInst->pszCurrFName);
    if (pInst->pszTag) free(pInst->pszTag);
    if (pInst->pszStateFile) free(pInst->pszStateFile);
...
static void do_polling(void) {
    ...
    while (glbl.GetGlobalInputTermState() == 0) {
        do {
            ...
            for (pInst = confRoot; pInst != NULL; pInst = pInst->next) {
                ...
                if (temp && stat((const char *)temp, &sb) == 0 && S_ISREG(sb.st_mode) &&
                    ...
                    DBGPRINTF(
                        "imtuxedoulog: timed file : rotation reach "
                ...
                /* let's poll the file */
                if (pInst->pszCurrFName != NULL) pollFile(pInst, &bHadFileData);

                DBGPRINTF("imtuxedoulog: do_polling end for '%s'\n", pInst->pszUlogBaseName);
                if (pInst->iPersistStateInterval == -1) persistStrmState(pInst);
            }
         ...
         */
        if (glbl.GetGlobalInputTermState() == 0) srSleep(iPollInterval, 10);
    }
}
...
BEGINnewInpInst
    ...
    if ((iRet = lstnAdd(inst)) != RS_RET_OK) {
        LogError(0, iRet, "add input %s to list failed", inst->pszUlogBaseName);
        lstnDel(inst);
        ABORT_FINALIZE(iRet);
    }
...
static inline void std_checkRuleset_genErrMsg(__attribute__((unused)) modConfData_t *modConf, instanceConf_t *inst) {
    LogError(0, NO_ERRCODE,
             "imtuxedoulog: ruleset '%s' for ULOG base %s not found - "
             "using default ruleset instead",
             inst->pszBindRuleset, inst->pszUlogBaseName);
}

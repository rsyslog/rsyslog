...
DEFobjCurrIf(glbl) DEFobjCurrIf(strm) DEFobjCurrIf(statsobj)
...
static rsRetVal setDynaFileCacheSize(void __attribute__((unused)) * pVal, int iNewVal) {
    ...
    if (iNewVal < 1) {
        ...
        errno = 0;
        parser_errmsg("DynaFileCacheSize maximum is 1,000 (%d given), changed to 1,000.", iNewVal);
        iRet = RS_RET_VAL_OUT_OF_RANGE;
        iNewVal = 1000;
    }
...
static rsRetVal fsCheck(instanceData *__restrict__ const pData, const uchar *__restrict__ const fileName) {
    DEFiRet;
    struct statvfs stat;
    char *pathcopy;
    const char *path;
...
finalize_it:
    if (pathcopy != NULL) free(pathcopy);
    RETiRet;
}
// </kortemik>
 ...
 */
static rsRetVal prepareDynFile(instanceData *__restrict__ const pData, const uchar *__restrict__ const newFileName) {
...
static void janitorChkDynaFiles(instanceData *__restrict__ const pData) {
    ...
    for (i = 0; i < pData->iCurrCacheSize; ++i) {
        ...
        if (pCache[i]->nInactive >= pData->iCloseTimeout) {
            ...
            dynaFileDelCacheEntry(pData, i, 1);
            if (pData->iCurrElt >= 0) {
                if ((uint)(pData->iCurrElt) == i) pData->iCurrElt = -1; /* no longer available! */
            }
        } else {

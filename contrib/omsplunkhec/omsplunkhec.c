...
static rsRetVal ATTR_NONNULL() writeDataError(wrkrInstanceData_t *const pWrkrData, instanceData *const pData, uchar *const reqmsg);
static void ATTR_NONNULL() curlCleanup(wrkrInstanceData_t *const pWrkrData);
static rsRetVal ATTR_NONNULL(1, 2) curlPost(wrkrInstanceData_t *pWrkrData, uchar *message, int msglen, const int nmsgs __attribute__((unused)));
...
static rsRetVal createBaseUrl(const char*const serverParam, const int defaultPort, const sbool useHttps, uchar **baseUrl);
...
checkResult(wrkrInstanceData_t *pWrkrData, uchar *reqmsg)
{
	...
	// 500+ errors return RS_RET_SUSPENDED if NOT batch and should be retried
...
curlResponse(void *ptr, size_t size, size_t nmemb, void *userdata)
{
	...
	if((buf = realloc(pWrkrData->reply, newlen + 1)) == NULL) {
		LogError(errno, RS_RET_ERR, "omsplunkhec: realloc failed in curlResult");
		return 0; 
	}
...
createApiHeader(char* token, uchar** headerBuf)
{
	...
	answer = es_addBuf(&header, (char *) HTTP_HEADER_CONTENT_JSON, strlen( (char *) HTTP_HEADER_CONTENT_JSON));
	if(answer == 0) answer = es_addChar(&header, ',');
	if(answer == 0) answer = es_addChar(&header, '\n');
	if(answer == 0) answer = es_addBuf(&header, (char *) HTTP_HEADER_AUTHORIZATION, strlen((char *) HTTP_HEADER_AUTHORIZATION));
...
CODESTARTnewActInst
	...
	if (pData->template == NULL){
		...
		CHKiRet(OMSRsetEntry(*ppOMSR, 0,(uchar*) strdup("StdJSONFmt"), OMSR_NO_RQD_TPL_OPTS));
	} else {
		iNumTpls = 1;
		CODE_STD_STRING_REQUESTnewActInst(iNumTpls)
...
CODESTARTdoAction
	...
	DBGPRINTF("omsplunkhec: doAction, pWrkrData->pData->batchMaxBatchSize = %d\n", (int)pWrkrData->pData->batchMaxBatchSize);
	...
	if (pWrkrData->pData->batch) {
		...
		DBGPRINTF("omsplunkhec: doAction, pWrkrData->batch.nmemb = %d\n", (int)pWrkrData->batch.nmemb);
		DBGPRINTF("omsplunkhec: doAction, batch_size = %d\n", (int)batch_size);
		DBGPRINTF("omsplunkhec: doAction, batch_size + nBytes = %d    | pWrkrData->pData->batchMaxBatchBytes = %d\n", (int)(batch_size + nBytes),(int)pWrkrData->pData->batchMaxBatchBytes);

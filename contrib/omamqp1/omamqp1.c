...
BEGINfreeInstance
	CODESTARTfreeInstance
	{
		...
		if (pData->reactor)
			pn_decref(pData->reactor);
		if (pData->handler)
			pn_decref(pData->handler);
		if (pData->message)
			pn_decref(pData->message);
	}
ENDfreeInstance
...
{
	if (pConfig->url)
		pn_url_free(pConfig->url);
	if (pConfig->username)
		free(pConfig->username);
	if (pConfig->password)
		free(pConfig->password);
	if (pConfig->target)
		free(pConfig->target);
	if (pConfig->templateName)
		free(pConfig->templateName);
	memset(pConfig, 0, sizeof(configSettings_t));
}
...
static void _poll_command(protocolState_t *ps)
{
	...
	case COMMAND_SEND:
		if (ps->delivery)
			break; // currently processing this command

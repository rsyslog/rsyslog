...
static const char *dns_rcodes[] = {"NoError",  // 0
                                   "FormErr",  // 1
                                   "ServFail",  // 2
                                   "NXDomain",  // 3
                                   "NotImp",  // 4
...
static const char *dns_types2[] = {"TKEY", "TSIG", "IXFR", "AXFR", "MAILB",    "MAILA", "*",
                                   "URI",  "CAA",  "AVC",  "DOA",  "AMTRELAY", NULL};
/* Part 3, since 32768. To prevent useless large buffer in memory */
static const char *dns_types3[] = {"TA", "DLV", NULL};
...
data_ret_t *dns_parse(const uchar *packet, int pktSize, struct json_object *jparent) {
    ...
    while (query_cpt < query_count && packet_ptr < end_packet) {
        ...
        domain_query[cpt] = '\0';
        if (cpt) packet_ptr++;  // pass the last \0, only if query was not empty
    ...
    if (response_flag) is_reponse = TRUE;

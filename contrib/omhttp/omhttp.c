 ...
 */
static rsRetVal serializeBatchJson(wrkrInstanceData_t *pWrkrData, char **batchBuf) {
    fjson_object *msgObj = NULL;
    size_t numMessages = pWrkrData->batch.nmemb;
    ...
    if (buff == NULL) ABORT_FINALIZE(RS_RET_ERR);


    for (size_t i = 0; i < numMessages; i++) {
        msgObj = fjson_tokener_parse((char *)pWrkrData->batch.data[i]);
        if (msgObj == NULL) {
            LogError(0, NO_ERRCODE, "omhttp: serializeBatchJson failed to parse %s as json, ignoring it",
                     pWrkrData->batch.data[i]);
            continue;
        }
    ...
    if (es_strlen(buff) > 0) {
        ...
        *batchBuf = NULL;
        LogError(0, NO_ERRCODE, "omhttp: serializeBatchJson: created empty batch buffer, no valid messages found");
    }

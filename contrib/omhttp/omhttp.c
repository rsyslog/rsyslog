...
static rsRetVal ATTR_NONNULL() checkConn(wrkrInstanceData_t *const pWrkrData) {
    ...
    if (pWrkrData->pData->lastHealthCheck != NULL && pWrkrData->pData->healthCheckTimeDelay != -1) {
        if (pWrkrData->pData->lastHealthCheck[pWrkrData->serverIndex] != 0 &&
            now - pWrkrData->pData->lastHealthCheck[pWrkrData->serverIndex] < pWrkrData->pData->healthCheckTimeDelay) {
...
BEGINnewActInst
    ...
    if (servers != NULL) {
        ...
        pData->lastHealthCheck = malloc(servers->nmemb * sizeof(time_t));
        if (pData->lastHealthCheck == NULL) {
            LogError(0, RS_RET_OUT_OF_MEMORY, "omhttp: unable to allocate memory for lastHealthCheck array");
            ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
        }
        ...
        for (i = 0; i < servers->nmemb; ++i) {
            ...
            free(serverParam);
            serverParam = NULL;
            pData->lastHealthCheck[i] = time(NULL);
        }
    } else {

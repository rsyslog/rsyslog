...
CODESTARTfreeCnf
	for(inst = pModConf->root ; inst != NULL ; ) {
		...
		if(inst->ca_cert_bundle != NULL)
			free(inst->ca_cert_bundle);
		if(inst->ca_cert_dir != NULL)
			free(inst->ca_cert_dir);
		if(inst->client_cert != NULL)
			free(inst->client_cert);
		if(inst->client_key != NULL)

...
static struct query_doc* create_query(struct queryopt* opt) {
    ...
    if (opt->bsever == 1) {
        bson_append_int32(query_what, "syslog_sever", 12, opt->e_sever);
    }
    if (opt->blevel == 1) {
    ...
    if (opt->bdate == 1) {
        date_what = bson_new();
        bson_init(date_what);
        bson_append_document_begin(query_what, "time_rcvd", 9, date_what);
        if (opt->bdatef == 1) {
            tm.tm_isdst = -1;
...
static struct db_cursor* launch_query(struct queryopt* opt,
    ...
    const bson_t* opts = BCON_NEW("skip", BCON_INT32(opt->e_skip), "limit", BCON_INT32(opt->e_ret));
    ...
    out->cursor = mongoc_collection_find_with_opts(db_coll->collection, qu_doc->query, opts, NULL);
#else /* !MONGOC_CHECK_VERSION (1, 5, 0) */
    out->cursor = mongoc_collection_find(db_coll->collection, MONGOC_QUERY_NONE, (uint32_t)opt->e_skip,
                                         (uint32_t)opt->e_ret, 0, qu_doc->query, s_doc->select, NULL);
#endif /* MONGOC_CHECK_VERSION (1, 5, 0) */
    if (!out->cursor) {
        perror("mongo_sync_cmd_query()");
...
static int cursor_next(struct db_cursor* db_c, struct results* res) {
    if (mongoc_cursor_next(db_c->cursor, &res->result)) return true;
    return false;
}
...
static struct db_collection* get_collection(struct db_connect* db_conn) {
    ...
    coll->collection = mongoc_client_get_collection(db_conn->conn, "syslog", "log");
    return coll;
}

...
---------------------------------------
Manage (GuardTime) KSI Signed Log Files
---------------------------------------
...
It specifically supports the GuardTime KSI Signature provider.

The *rsksiutil* tool is the primary tool to verify log file signatures,
...
are specified. The corresponding *signature* files (ending on ".ksisig") must also
...
signature file is logfile.ksisig and the temporary work file is named
logfile.ksisig.new. When extending finished successfully, the original
signature file (logfile.ksisig in our example) is renamed with the .old
postfix (logfile.ksisig.old) and the temporary file written under the
...
**rsksiutil --verify logfile**

This verifies the file "logfile" via its associated signature file
"logfile.ksisig". If errors are detected, these are reported to stderr.
Otherwise, rsksiutil terminates without messages.

**rsksiutil --dump logfile.ksisig**

This dumps the content of the signature file "logfile.ksisig". The
